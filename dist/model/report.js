...
export class ReportBuilder {
    ...
    paths = [];
    operations = [];
    limitsReport = { applied: [], truncations: [] };
    selection = null;
    constructor(input) {
        this.input = input;
    ...
    edge(input) {
        ...
        if (!evidenceIds.length)
            throw new ModelError(`Edge ${input.kind} ${input.from} -> ${input.to} has no evidence`);
    ...
    path(input) {
        const raw = { id: pathId({ occurrenceIds: input.occurrenceIds, edgeIds: input.edgeIds }),
    ...
    operation(input) {
        const raw = { id: operationId({ event: input.event, eventId: input.eventId, listenerId: input.listenerId }),
    ...
    select(selection) { this.selection = selection; }
    build() {
        const selection = this.selection;
        if (!selection)
            throw new ModelError('The selected candidate must be set before the report is built');
        const edges = [...this.edges.values()].sort(byId);
        const confidenceOf = new Map(edges.map(edge => [edge.id, edge.confidence]));
        const weakest = (ids) => weakestConfidence(ids.map(id => confidenceOf.get(id) ?? 'unresolved'));
        const paths = this.paths.map(raw => {
        ...
        const operations = this.operations.map(raw => ({
        ...
        const overall = combineCoverage([pathCoverage, ...events.map(item => item.coverage),
            ...(openGaps.length ? ['partial'] : []), ...(truncations.length ? ['partial'] : [])]);
        ...
        return {
            ...
            coverage: { overall, paths: pathCoverage, events, gaps, gapCounts: [...gapCounts].sort(([a], [b]) => a < b ? -1 : a > b ? 1 : 0).map(([code, count]) => ({ code, count })),
