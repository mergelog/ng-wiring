...
export function validateReport(report) {
    ...
    for (const gap of report.coverage.gaps) {
        ...
        if (gap.resolvedBy !== null) {
            ...
            check(gap.resolvedReason === null, `Gap ${gap.id} carries a fill-in reason without resolvedBy`);
    }
    const counted = [...unrelatedCounts].sort(([a], [b]) => a < b ? -1 : a > b ? 1 : 0)
        .map(([code, count]) => ({ code, count }));
    check(JSON.stringify(report.coverage.gapCounts) === JSON.stringify(counted), 'coverage.gapCounts does not count the unrelated gaps by code');
    for (const gap of report.coverage.gaps.filter(item => item.relation === 'related' && !item.resolvedBy)) {
        check(report.coverage.reasons.includes(`${gap.code}: ${gap.message}`), `Related gap ${gap.id} is missing from coverage.reasons`);
    }
