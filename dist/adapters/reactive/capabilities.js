...
const entry = (draft) => ({ member: null, form: 'function', framework: null,
...
const registry = [
    ...
    entry({ matcherId: 'signals-events/injectDispatch', semanticId: 'event-dispatch:named', contracts: ['R12', 'R14'],
        module: EVENTS, export: 'injectDispatch', framework: 'signal-store',
        note: 'the self/parent/global scope is written at this call site, so R14 delivery starts here' }),
    entry({ matcherId: 'signals-events/Dispatcher.dispatch', semanticId: 'event-dispatch:direct', contracts: ['R12', 'R14'],
    ...
    entry({ matcherId: 'rxjs/of', semanticId: 'call:observable-creation', contracts: ['R08', 'R15'], module: 'rxjs',
        export: 'of' }),
    entry({ matcherId: 'rxjs/from', semanticId: 'call:observable-conversion', contracts: ['R08', 'R15'], module: 'rxjs',
        export: 'from', note: 'an array, an iterable or a Promise; a Promise settles asynchronously' }),
    entry({ matcherId: 'rxjs/distinctUntilChanged', semanticId: 'reactive-link:distinct', contracts: ['R08', 'R15'],
        module: 'rxjs', export: 'distinctUntilChanged' }),
    entry({ matcherId: 'ngrx-operators/tapResponse', semanticId: 'reactive-link:response-branch',
        contracts: ['R08', 'R15'], module: '@ngrx/operators', export: 'tapResponse' }),
    entry({ matcherId: 'ngrx-operators/mapResponse', semanticId: 'reactive-link:response-branch',
        contracts: ['R08', 'R15'], module: '@ngrx/operators', export: 'mapResponse' }),
...
export function capabilities() { return registry; }
export function capabilityById(matcherId) {
    return registry.find(item => item.matcherId === matcherId);
}
export function capabilitiesForContract(contract) {
    return registry.filter(item => item.contracts.includes(contract));
}
export const CONTRACT_IDS = Array.from({ length: 16 }, (_unused, index) => `R${String(index + 1).padStart(2, '0')}`);
const slash = (value) => value.replaceAll('\\', '/');
const moduleMaps = new Map();
const fileModules = new WeakMap();
function exportTargets(value, into, key) {
    ...
    for (const [condition, nested] of Object.entries(value)) {
        if (condition.startsWith('.'))
            exportTargets(nested, into, condition);
        else
            exportTargets(nested, into, key);
    }
}
/** Maps a declaration file back to the specifier that publishes it, so subpaths stay distinct. */
export function moduleIdForFile(context, fileName) {
    let cache = fileModules.get(context);
    if (!cache) {
        cache = new Map();
        fileModules.set(context, cache);
    }
    ...
    if (!map) {
        ...
        if (text) {
            try {
                exportTargets(JSON.parse(text).exports, map, '.');
            }
...
function locate(context, symbol) {
    if (!symbol)
        return null;
...
export function matchIdentifier(context, node, forms = ['function', 'class']) {
...
export function matchMember(context, receiver, member) {
...
export function unsupportedImports(context, file) {
    const t = context.toolchain.typescript;
    const result = [];
    for (const statement of file.statements) {
        if (!t.isImportDeclaration(statement) || !t.isStringLiteralLike(statement.moduleSpecifier))
            continue;
        const specifier = statement.moduleSpecifier.text;
        const byModule = registry.find(item => item.form === 'module' && item.support === 'unsupported' &&
            item.module === specifier);
        if (byModule) {
            result.push({ capability: byModule, module: specifier, specifier, location: statement });
            continue;
        }
        const clause = statement.importClause?.namedBindings;
        if (!clause || !t.isNamedImports(clause))
            continue;
        for (const element of clause.elements) {
            ...
            if (capability)
                result.push({ capability, module: specifier, specifier: name, location: element });
        }
    }
    return result;
}
/** Checks the registry against the installed packages so a renamed or removed export cannot pass unnoticed. */
export function auditCapabilities(context) {
    const t = context.toolchain.typescript;
    const problems = [];
    const exportsOf = new Map();
    const namesFor = (module) => {
        const cached = exportsOf.get(module);
        if (cached !== undefined)
            return cached;
        const resolved = t.resolveModuleName(module, `${context.workspaceRoot}/index.ts`, context.compilerOptions, { fileExists: file => t.sys.fileExists(file), readFile: file => t.sys.readFile(file) });
    ...
    for (const capability of registry) {
        if (capability.form === 'module' || !capability.export)
            continue;
