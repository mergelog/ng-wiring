const empty = () => ({ ids: new Set(), complete: true, reasons: [] });
export class ScopeResolver {
    ...
    scopeOf(component) {
        ...
        if (component.standalone)
            this.addRefs(component.imports, result);
        else {
            const modules = this.declaring.get(component.id) ?? [];
            if (!modules.length) {
                result.complete = false;
                result.reasons.push(`No NgModule declares ${component.id}`);
            }
            if (modules.length > 1) {
                result.complete = false;
                result.reasons.push(`Multiple NgModules declare ${component.id}`);
            }
    ...
    exportsOf(id) {
        ...
        if (!declaration || declaration.kind !== 'module')
            return result;
    ...
    addRefs(ids, scope) {
        for (const id of ids) {
            ...
            if (declaration.kind === 'module') {
                ...
                for (const exported of exports.ids)
                    scope.ids.add(exported);
                if (!exports.complete) {
                    scope.complete = false;
                    scope.reasons.push(...exports.reasons);
                }
            }
            else
                scope.ids.add(id);
