...
function relativeSpan(context, span) {
...
function declarationOf(context, node) {
    const symbol = symbolOf(context, node);
    return symbol?.valueDeclaration ?? symbol?.declarations?.[0];
}
function fromAngular(context, node, packageName) {
    return symbolOf(context, node)?.declarations?.some(declaration => slash(declaration.getSourceFile().fileName).includes(`/node_modules/@angular/${packageName}/`)) ?? false;
}
function calleeName(context, call) {
    const t = context.toolchain.typescript;
...
function routerModuleCall(context, node, method) {
    const t = context.toolchain.typescript;
    const call = unwrap(t, node);
    if (!t.isCallExpression(call) || !t.isPropertyAccessExpression(call.expression) || call.expression.name.text !== method)
        return undefined;
    const owner = classAt(context, call.expression.expression);
    if (!owner || owner.name?.text !== 'RouterModule')
        return undefined;
    if (!slash(owner.getSourceFile().fileName).includes(ANGULAR_ROUTER))
        return undefined;
    return call.arguments[0];
}
/** The receiver is the installed @angular/router Router, not an unrelated object with the same method name. */
function isRouterReceiver(context, receiver) {
    const symbol = context.checker.getTypeAtLocation(receiver).getSymbol();
    return symbol?.getName() === 'Router' && (symbol.declarations?.some(declaration => slash(declaration.getSourceFile().fileName).includes(ANGULAR_ROUTER)) ?? false);
}
function isConstVariable(context, declaration) {
    const t = context.toolchain.typescript;
...
function arrayLiteralOf(context, expression, active = new Set()) {
    const t = context.toolchain.typescript;
...
function moduleDefaultExport(context, call) {
    const t = context.toolchain.typescript;
    const specifier = call.arguments[0];
    if (!specifier || !t.isStringLiteralLike(specifier))
        return undefined;
    const resolved = t.resolveModuleName(specifier.text, call.getSourceFile().fileName, context.compilerOptions, t.sys).resolvedModule;
...
export function lazyTarget(context, expression, depth = 0) {
    ...
    if (t.isArrowFunction(node) || t.isFunctionExpression(node)) {
        if (!t.isBlock(node.body))
            return lazyTarget(context, node.body, depth + 1);
    ...
    if (t.isAwaitExpression(node))
        return lazyTarget(context, node.expression, depth + 1);
    if (t.isCallExpression(node) && node.expression.kind === t.SyntaxKind.ImportKeyword)
        return moduleDefaultExport(context, node);
    if (t.isCallExpression(node) && t.isPropertyAccessExpression(node.expression) && node.expression.name.text === 'then') {
        ...
        if (callback && (t.isArrowFunction(callback) || t.isFunctionExpression(callback)))
            return lazyTarget(context, callback, depth + 1);
        return undefined;
    }
    if (t.isPropertyAccessExpression(node))
        return declarationOf(context, node.name);
    if (t.isIdentifier(node))
        return declarationOf(context, node);
    return undefined;
}
function componentIdOf(context, catalog, expression) {
    ...
    while (queue.length) {
        ...
        const visit = (node) => {
            if ((t.isImportDeclaration(node) || t.isExportDeclaration(node)) && node.moduleSpecifier &&
                t.isStringLiteralLike(node.moduleSpecifier))
                specifiers.push(node.moduleSpecifier.text);
            if (t.isCallExpression(node) && node.expression.kind === t.SyntaxKind.ImportKeyword &&
                node.arguments[0] && t.isStringLiteralLike(node.arguments[0]))
        ...
        for (const specifier of specifiers) {
            ...
            if (target && context.sourceFiles.includes(target))
                queue.push(target);
        }
    }
    return seen;
}
function providerList(context, expression, gaps, active = new Set()) {
    const t = context.toolchain.typescript;
    const node = unwrap(t, expression);
    if (active.has(node))
        return [];
...
function configProviders(context, expression, gaps, active = new Set()) {
    const t = context.toolchain.typescript;
    const node = unwrap(t, expression);
    if (active.has(node))
        return [];
    ...
    if (t.isCallExpression(node) && t.isIdentifier(node.expression)) {
        ...
        if (body && context.sourceFiles.includes(body.getSourceFile().fileName)) {
            ...
            if (result)
                return configProviders(context, result, gaps, active);
        }
    }
    if (t.isIdentifier(node) || t.isPropertyAccessExpression(node)) {
        const declaration = declarationOf(context, t.isPropertyAccessExpression(node) ? node.name : node);
...
function rootRoutesFromProviders(context, providers) {
    const t = context.toolchain.typescript;
    const found = [];
    for (const provider of providers) {
        if (!t.isCallExpression(provider))
            continue;
...
function moduleRootRoutes(context, catalog, moduleId, gaps, active = new Set()) {
    ...
    if (!declaration) {
        gaps.push(`NgModule ${moduleId} is outside the analysis context`);
        return [];
    }
    const found = [];
    ...
    if (providers)
        found.push(...rootRoutesFromProviders(context, providerList(context, providers, gaps)));
    return found;
}
function findBootstraps(context, catalog) {
    const t = context.toolchain.typescript;
    const output = [];
            ...
            const visit = (node) => {
                if (t.isCallExpression(node)) {
                    ...
                    if (name === 'bootstrapApplication' && t.isIdentifier(node.expression) &&
                        ...
                        if (!componentId)
                            gaps.push('Bootstrap component is not resolvable to a catalogued component');
                            ...
                            if (list && t.isArrayLiteralExpression(list)) {
                                for (const element of list.elements) {
                                    ...
                                    if (id)
                                        ...
                                        gaps.push(`Unresolved bootstrap entry ${element.getText().slice(0, 60)}`);
                                }
                            }
                            else
                                gaps.push(`NgModule ${moduleId} has no statically resolvable bootstrap array`);
...
function configIdFor(context, expression) {
...
function occurrenceIdFor(context, definition, loaders) {
...
function joinPattern(parent, segment, unresolved) {
...
export function buildRouteGraph(context, catalog, maze) {
    const t = context.toolchain.typescript;
    ...
    const walk = (input) => {
        ...
        for (const [order, element] of input.array.elements.entries()) {
            if (occurrences.length >= MAX_OCCURRENCES) {
                gaps.push('Route occurrence limit reached');
                return;
            }
            ...
            if (pathProperty.present && pathProperty.value === null)
                localGaps.push('Route path is not statically resolvable');
            ...
            for (const guard of ['canActivate', 'canActivateChild', 'canMatch', 'canLoad', 'canDeactivate']) {
                ...
                if (value)
                    conditions.push({ kind: 'guard', text: `${guard} ${value.getText().replace(/\s+/g, ' ').slice(0, 80)}`, span: spanOf(value) });
            }
            const providers = getProperty(t, route, 'providers');
            if (providers)
                conditions.push({ kind: 'providers', text: 'route providers change the injector for this subtree', span: spanOf(providers) });
            ...
            if (componentProperty) {
                ...
                if (!componentId) {
                    ...
                    localGaps.push('Route component is not a catalogued component');
                }
            }
            else if (loadComponent) {
                componentId = classIdOf(context, lazyTarget(context, loadComponent)) ?? null;
                if (componentId && !catalog.declarations.has(componentId))
                    componentId = null;
                if (!componentId) {
                    componentUnresolved = true;
                    localGaps.push('loadComponent target is not resolvable');
                }
            }
            const occurrence = {
            ...
            if (children) {
                ...
                if (array)
                    walk({ array, configId: input.configId, rooted: input.rooted, loaders: [...input.loaders, spanOf(children)], parent: occurrence, chain });
                else
                    localGaps.push('children is not a statically resolvable route array');
            }
            ...
            if (loadChildren) {
                ...
                let array;
                if (target && isConstVariable(context, target) && target.initializer)
                    array = arrayLiteralOf(context, target.initializer);
                    ...
                    if (list && t.isArrayLiteralExpression(list)) {
                        ...
                        for (const item of list.elements) {
                            ...
                            if (childArray)
                                walk({ array: childArray, configId: input.configId, rooted: input.rooted, loaders: [...loaders, spanOf(item)], parent: occurrence, chain });
                            else
                                localGaps.push('RouterModule.forChild routes are not statically resolvable');
                        }
                        if (!forChild)
                            localGaps.push(`Lazy NgModule ${moduleId} declares no RouterModule.forChild routes`);
                    }
                    else
                        localGaps.push('Lazy NgModule imports are not statically resolvable');
                }
                if (array)
                    walk({ array, configId: input.configId, rooted: input.rooted, loaders, parent: occurrence, chain });
                else if (!target)
                    localGaps.push('loadChildren target is not resolvable');
            }
        }
    };
    const found = findBootstraps(context, catalog);
    const bootstraps = found.map(item => item.bootstrap);
    if (!context.entry.length)
        gaps.push('No application entry is known; no root router configuration can be attached to a bootstrap');
    else if (!bootstraps.length)
        gaps.push('No bootstrapApplication or bootstrapModule call is reachable from the selected entry');
    for (const { bootstrap, rootRoutes } of found) {
        for (const expression of rootRoutes) {
            ...
            if (!array) {
                gaps.push(`Root route configuration is not a statically resolvable array at ${spanOf(expression).file}:${spanOf(expression).line}`);
                continue;
            }
    ...
    const registerLoose = (expression, kind) => {
        const array = arrayLiteralOf(context, expression);
        if (!array || visitedArrays.has(array))
            return;
        const id = configIdFor(context, expression);
        if (configs.get(id)?.rooted)
            return;
    ...
    for (const file of context.sourceFiles) {
        ...
        const visit = (node) => {
            if (t.isCallExpression(node)) {
                ...
                for (const method of ['forRoot', 'forChild']) {
                    const routes = routerModuleCall(context, node, method);
                    if (routes)
                        loose.push({ expression: routes, kind: method });
                }
            ...
            if (t.isVariableDeclaration(node) && node.type && node.initializer &&
                /^(?:Routes|Route\[\])$/.test(node.type.getText()))
                loose.push({ expression: node.initializer, kind: 'routes-array' });
            t.forEachChild(node, visit);
        };
        visit(source);
        for (const item of loose)
            registerLoose(item.expression, item.kind);
    }
    ...
    const bootstrapByComponent = new Map();
    ...
    for (const route of maze?.routes ?? []) {
        ...
        if (!matched)
            diagnostics.push(`ngmaze route ${route.path} -> ${route.target} at ${route.location.file}:${route.location.line} was not reconstructed from source`);
    }
...
export function resolveOutletPlacement(context, graph, index, occurrence) {
    ...
    let hostIds;
    if (hostRoute?.componentId)
        hostIds = [hostRoute.componentId];
    else {
        ...
        if (!bootstrap)
            return [{ kind: 'unresolved', reason: `Route ${occurrence.pattern} has no resolved bootstrap host` }];
        if (!bootstrap.componentIds.length)
            return [{ kind: 'unresolved', reason: `Bootstrap ${bootstrap.id} has no resolved component` }];
    ...
    for (const hostId of hostIds) {
        ...
        if (!outlets.length) {
            results.push({ kind: 'unresolved', reason: `${hostId} declares no <router-outlet> for route ${occurrence.pattern}` });
            continue;
        }
        ...
        for (let ancestor = element.parent; ancestor; ancestor = ancestor.parent) {
            if (ancestor.component) {
                blocked = `<router-outlet> is projected into ${ancestor.component}; the router context is unresolved`;
                break;
            }
            if (ancestor.tag === 'ng-template' && ancestor.references.length) {
                blocked = `<router-outlet> is declared inside TemplateRef #${ancestor.references[0]}; the router context is unresolved`;
                break;
            }
            if (ancestor.node.constructor.name === 'Template')
                conditions.push(`embedded view ${ancestor.tag} exists`);
        }
        if (element.repeated)
            conditions.push('@for iteration exists; individual row is not identified');
        if (blocked) {
            results.push({ kind: 'unresolved', reason: blocked });
            continue;
        }
