...
function sourceId(context, node) {
...
function classSymbol(context, owner, name) {
    return context.checker.getTypeAtLocation(owner.node).getProperty(name) ?? null;
}
function referenceTarget(context, catalog, binding) {
    const element = binding.element;
    if (!element)
        return { name: '', kind: 'unknown', origin: 'template reference without host', symbol: null,
    ...
    if (!binding.value && element.tag === 'ng-template')
        return { name: '', kind: 'reference', origin: 'template-ref',
            symbol: null, targetIds: [], conditions: ['TemplateRef requires an insertion to appear in the DOM'], diagnostics: [] };
    if (!binding.value && element.tag === 'ng-container')
        return { name: '', kind: 'unknown', origin: 'ng-container',
            symbol: null, targetIds: [], conditions: [], diagnostics: ['ng-container has no DOM element instance'] };
    if (!binding.value)
        return { name: '', kind: 'reference', origin: element.component ? 'component-host' : 'dom-element',
            symbol: null, targetIds: element.component ? [element.component] : [], conditions: [], diagnostics: [] };
    const matches = [element.component, ...element.directives].filter((id) => !!id).filter(id => {
        const declaration = catalog.declarations.get(id);
        if (!declaration)
            return catalog.external.get(id)?.exportAs.includes(binding.value) ?? false;
...
function queryOf(context, index, element, member) {
    const t = context.toolchain.typescript;
    const node = member.valueDeclaration;
    if (!node || !t.isPropertyDeclaration(node) || !node.initializer || !t.isCallExpression(node.initializer))
        return null;
    ...
    if (!['viewChild', 'contentChild', 'viewChildren', 'contentChildren'].includes(name ?? '') ||
        !symbol?.declarations?.some(part => slash(part.getSourceFile().fileName).includes('/node_modules/@angular/core/')))
        return null;
    const kind = name;
    ...
    const visible = kind.startsWith('view') ? index.byOwner.get(element.owner.id) ?? [] :
        index.elements.filter(candidate => {
            for (let parent = candidate.parent; parent; parent = parent.parent)
                if (parent.component === element.owner.id)
                    return true;
            return false;
        });
    let candidates = [];
    if (selector && t.isStringLiteralLike(selector)) {
        ...
        if (id)
            candidates = visible.filter(candidate => candidate.component === id || candidate.directives.includes(id));
    }
    ...
    if (!required)
        conditions.push('query may be undefined before creation or when no match exists');
    else
        conditions.push('required query can fail if no matching view is created');
    if (candidates.length !== 1 && !kind.endsWith('Children'))
        conditions.push('query instance is not uniquely identified');
    if (read)
        conditions.push(`read token ${read} changes the returned instance`);
...
export function resolveTemplateExpressions(element, context, catalog, index) {
    ...
    const memberRef = (name) => {
        ...
        if (!symbol)
            return { name, kind: 'unknown', origin: owner.id, symbol: null, targetIds: [], conditions: [],
    ...
    const resolve = (name, explicitThis = false) => {
        if (!explicitThis && name === '$event' && eventContext)
            return { name, kind: 'event', origin: 'listener payload', symbol: null,
                targetIds: [], conditions: [], diagnostics: [] };
        if (!explicitThis && element.lexical.has(name)) {
            const binding = element.lexical.get(name);
            if (binding.kind === 'reference')
                return { ...referenceTarget(context, catalog, binding), name };
    ...
    const visitor = new class extends ng.RecursiveAstVisitor {
        ...
        visitPipe(ast, ctx) {
            const pipe = [...scope.ids].map(id => catalog.declarations.get(id) ?? catalog.external.get(id)).filter(item => item?.kind === 'pipe' && item.selector === ast.name);
        ...
        recordCall(receiver, optional) {
            if (receiver instanceof ng.PropertyRead || receiver instanceof ng.SafePropertyRead) {
                ...
                if (base instanceof ng.ImplicitReceiver || base instanceof ng.ThisReceiver)
                    ...
                    resolved = { name: receiver.name, kind: 'unknown', origin: 'computed or chained receiver',
                ...
                if (!symbol && resolved.kind === 'reference' && resolved.targetIds.length === 1) {
                    ...
                    if (instance)
                        symbol = context.checker.getTypeAtLocation(instance.node).getProperty(receiver.name) ?? null;
                }
                ...
                if (optional || receiver instanceof ng.SafePropertyRead)
                    conditions.push('optional receiver may be null or undefined');
                if (resolved.kind === 'reference' && resolved.targetIds.length !== 1)
                    conditions.push('reference instance is not unique');
