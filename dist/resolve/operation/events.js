...
function isFormsModelUpdate(subscription) {
...
function hostEvents(context, catalog, element) {
    const t = context.toolchain.typescript;
    const result = [];
    for (const id of [element.component, ...element.directives].filter((id) => !!id)) {
        ...
        if (host && t.isPropertyAssignment(host) && t.isObjectLiteralExpression(host.initializer)) {
            for (const part of host.initializer.properties)
                if (t.isPropertyAssignment(part)) {
                    const event = part.name.getText().replaceAll(/["'()]/g, '');
                    if (part.name.getText().includes('('))
                        result.push({ event, handler: part.initializer.getText(), subscription: id });
                }
        }
        const seen = new Set();
        const scan = (node) => {
            ...
            for (const member of node.members)
                for (const decorator of t.canHaveDecorators(member) ? t.getDecorators(member) ?? [] : []) {
                    ...
                    if (!t.isIdentifier(callee) || callee.text !== 'HostListener')
                        continue;
                    const arg = decorator.expression.arguments[0];
                    if (arg && t.isStringLiteralLike(arg))
                        result.push({ event: arg.text, handler: member.name?.getText() ?? '', subscription: id });
                }
...
function captureListeners(selected, context, eventFilter) {
    const t = context.toolchain.typescript;
    const result = [];
    const isHostRef = (node) => {
        if (!t.isPropertyAccessExpression(node) || node.name.text !== 'nativeElement')
            return false;
        ...
        if (!t.isIdentifier(call.expression) || call.expression.text !== 'inject')
            return false;
        const token = call.arguments[0];
        if (!token || !t.isIdentifier(token) || token.text !== 'ElementRef')
            return false;
        const angularSymbol = (name) => {
    ...
    const scan = (node) => {
        if (t.isCallExpression(node) && t.isPropertyAccessExpression(node.expression) && node.expression.name.text === 'addEventListener') {
            ...
            if (nameNode && t.isStringLiteralLike(nameNode) && handler) {
                if (eventFilter && nameNode.text !== eventFilter && baseName(nameNode.text) !== eventFilter)
                    return;
                ...
                if (capture) {
                    ...
                    if (global || host)
                        result.push({ selectedElement: selected, listenerElement: null,
    ...
    for (const member of selected.owner.node.members)
        scan(member);
    return result;
}
/** A DOM source and an Angular output are separate possible sources for the same spelling. */
export function resolveEventListeners(selected, context, catalog, eventFilter, placement) {
    const listeners = [];
    ...
    if (placement) {
        for (const step of placement.path.steps) {
            if (!step.displayParent || !['element', 'component-use'].includes(step.relation) || !step.span)
                continue;
            ...
            if (parent && !ancestry.includes(parent))
    ...
    for (const [depth, element] of ancestry.entries()) {
        ...
        for (const binding of [...template, ...hosted]) {
            ...
            const innerStops = ancestry.slice(0, depth).flatMap(item => item.eventStops.filter(stop => baseName(stop.event) === name));
            if (depth > 0 && !global && innerStops.some(stop => stop.definite))
                continue;
            ...
            if (depth > 0 && !event?.bubbles && !global)
                continue;
            if (!event && !global)
                conditions.push('DOM event bubbles/composed are unknown');
            if (modifiers.length)
                conditions.push(`event modifiers ${modifiers.join('.')} must match`);
            if (placementUnknown)
                conditions.push('actual DOM placement across component or projection boundary is unknown');
            ...
            if (event?.composed && depth > 0)
                conditions.push('shadow DOM retargeting may change the event target');
            if (global)
                conditions.push('global listener registration; no ancestor element is implied');
            const base = { selectedElement: selected, listenerElement: global ? null : element,
            ...
            if (global || event || depth === 0)
                listeners.push(base);
            else
                diagnostics.push(`Unknown DOM propagation for ${actualName} at ${element.span.file}:${element.span.line}`);
        }
    }
    listeners.push(...captureListeners(selected, context, eventFilter));
    const derivedEvents = [];
    if ((!eventFilter || eventFilter === 'click') && selected.tag === 'button')
        derivedEvents.push({
    ...
    if (listeners.some(listener => /\.\s*focus\s*\(/.test(listener.handler)))
        derivedEvents.push({
