...
function declarationsAt(context, node) {
    const t = context.toolchain.typescript;
...
function referencesOf(context, name, scope) {
    const t = context.toolchain.typescript;
    const targets = declarationsAt(context, name);
    const found = [];
    const visit = (node) => {
        if (t.isIdentifier(node) && node !== name &&
            declarationsAt(context, node).some(item => targets.includes(item)))
            found.push(node);
...
function callSitesOf(context, declaration) {
    const t = context.toolchain.typescript;
    const found = [];
    if (!declaration.name)
        return found;
    const visit = (node) => {
        if (t.isCallExpression(node)) {
            ...
            if (declarationsAt(context, callee).includes(declaration))
                found.push(node);
        }
        t.forEachChild(node, visit);
    };
...
function asyncPipeFields(context, owner) {
    ...
    if (parsed.errors?.length)
        return { fields, gap: `${owner.id}: template parse error` };
    ...
    const nodes = new class extends ng.TmplAstRecursiveVisitor {
        visitBoundText(text) { text.value.visit(expressions); }
        visitBoundAttribute(attribute) {
            attribute.value.visit(expressions);
        }
        visitBoundEvent(event) { event.handler.visit(expressions); }
        ...
        visitLetDeclaration(declaration) {
...
function consumptionOf(context, start, state, inputs) {
    const t = context.toolchain.typescript;
    let current = start;
    const stop = (node, reason) => {
    ...
    for (let step = 0; step < DEPTH; step++) {
        if (state.visited.has(current))
            return stop(current, 'the value path revisits an expression already followed');
        state.visited.add(current);
        const parent = current.parent;
        if (!parent)
            return stop(current, 'the created value leaves the analyzed expression');
        if (parenLike(t, parent)) {
            current = parent;
            continue;
        }
        ...
        if (member) {
            ...
            if (member.name === 'subscribe')
                return consume('subscribe', context, member.call, state, ['the subscription starts the request when this call runs',
                    'the request ends early if the subscription is closed before the response']);
            if (['then', 'catch', 'finally'].includes(member.name))
                return consume('promise-result', context, member.call, state, [`the ${member.name} callback continues after the request settles`]);
        ...
        if (t.isCallExpression(parent) && parent.arguments.includes(current)) {
            ...
            if (rxjs === 'firstValueFrom' || rxjs === 'lastValueFrom')
                return consume('promise-consume', context, parent, state, [`${rxjs} subscribes when it is called and settles a Promise ${rxjs === 'firstValueFrom' ? 'on the first value' : 'after completion'}`,
            ...
            if (matcher === 'angular/toSignal')
                return consume('to-signal', context, parent, state, ['toSignal subscribes when it is created and unsubscribes when its injection context is destroyed'], ['the toSignal call site must run in an injection context that is created on the selected path']);
            if (matcher === 'signals/rxMethod') {
                const record = inputs.methods?.methods.find(item => item.id === location(context, parent));
                return consume('rx-method', context, parent, state, ['the rxMethod pipeline runs once per call and once per notification of a Signal or Observable argument',
                    ...(record?.conditions ?? [])], [record ? `rxMethod ${record.name ?? record.id} is ${record.called ? 'called' : 'never called'}`
                        : 'no reactive method graph was supplied, so the rxMethod call is unconfirmed'], record?.called === true);
            }
            if (matcher === 'ngrx-effects/createEffect') {
                const effect = inputs.store?.effects.find(item => item.source === location(context, parent));
                return consume('effect-flattening', context, parent, state, ['the effect source must emit before the flattened request is subscribed', ...(effect?.conditions ?? [])], [effect ? `effect at ${effect.source} is ${effect.registered ? 'registered' : 'not registered'}`
                        : 'no Store graph was supplied, so the effect registration is unconfirmed'], effect?.registered === true);
            }
            if (matcher === 'signals-events/withEventHandlers') {
                ...
                return consume('event-handler', context, parent, state, ['the handler subscribes when the Store is created and ends when the Store is destroyed',
                    ...(lifetime?.conditions ?? [])], [lifetime && declaration ? `Store ${declaration.name} is ${lifetime.created ? 'created' : 'declared but never created'}`
                        : 'no SignalStore catalog was supplied, so the handler registration is unconfirmed'], lifetime?.created === true);
            }
            return stop(parent, `the value is passed to ${callee.getText()}, whose subscription behaviour is not modelled`);
        }
        if (t.isPropertyAssignment(parent) && parent.initializer === current) {
        ...
        if (assignedField) {
            const owner = t.findAncestor(parent, node => t.isClassDeclaration(node));
            if (!owner)
                return stop(parent, 'the class that holds the created value is unresolved');
            ...
            if (declaration) {
                ...
                if (template.gap)
                    state.gaps.push(template.gap);
                if (template.fields.has(name))
                    return consume('async-pipe', context, owner, state, ['the async pipe subscribes while the view exists and unsubscribes when the view is destroyed',
                        'the view that holds this binding must be created on the selected path'], [`${declaration.id} reads ${name} through the async pipe`]);
            }
            else
                state.gaps.push(inputs.catalog
        ...
        if (t.isExpressionStatement(parent))
            return stop(parent, 'the created value is discarded without a subscription');
...
export function callIndex(context) {
    ...
    const visit = (node) => {
        if (t.isCallExpression(node))
            index.set(location(context, node), node);
        t.forEachChild(node, visit);
    };
    for (const file of context.sourceFiles) {
        const source = context.program.getSourceFile(file);
...
export function resolveHttpStart(context, site, inputs = {}, index = callIndex(context), callPath) {
    ...
    if (site.transport === 'fetch')
        return { request: site, start: 'confirmed', stages,
    ...
    if (consumption.kind === 'none')
        return { request: site, start: 'candidate', stages, consumption,
            reason: consumption.gaps[0] ?? 'no subscription of the created Observable was found' };
    if (!consumption.registered)
        return { request: site, start: 'candidate', stages, consumption,
...
export function analyzeHttpFlows(context, catalog, inputs = {}) {
...
function traceHttpFromRoot(context, catalog, root, rootName, rootOwner, options, entryConditions = []) {
    ...
    const requestsIn = (body) => catalog.requests.filter(site => {
        const node = index.get(site.id);
    ...
    const visitMethod = (declaration, receiver, path, conditions, depth, argumentsAtCall = [], inheritedContainer = null) => {
        ...
        if (depth >= DEPTH) {
            add('boundary', receiver, 'call stack depth limit', declaration, path, conditions);
            return;
        }
        ...
        const visit = (node, localConditions, currentPath = path) => {
            if (expanded > LIMIT)
                return;
            ...
            if (t.isTryStatement(node)) {
                ...
                if (node.catchClause)
                    visit(node.catchClause.block, [...localConditions, 'exception thrown in try block'], currentPath);
                if (node.finallyBlock)
                    visit(node.finallyBlock, localConditions, currentPath);
                return;
            }
            if (t.isCallExpression(node)) {
                const callee = t.isPropertyAccessExpression(node.expression) ? node.expression.name : node.expression;
...
export function traceHttpFromStoreMethod(context, catalog, method, storeName, options = {}) {
