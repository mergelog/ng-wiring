...
function receiverName(context, node) {
    const t = context.toolchain.typescript;
    if (t.isPropertyAccessExpression(node) && node.expression.kind === t.SyntaxKind.ThisKeyword)
        return node.name.text;
    if (t.isIdentifier(node))
        return node.text;
    return null;
}
function subscription(context, call, place) {
    const t = context.toolchain.typescript;
    if (!t.isPropertyAccessExpression(call.expression) || call.expression.name.text !== 'subscribe')
        return null;
    ...
    if (!observableType.getSymbol()?.declarations?.some(d => d.getSourceFile().fileName.replaceAll('\\', '/')
        .includes('/node_modules/rxjs/')))
        return null;
    let source = call.expression.expression;
    const operators = t.isCallExpression(source) ? inspectPipe(context, source) : [];
    if (operators.length && t.isCallExpression(source) && t.isPropertyAccessExpression(source.expression))
    ...
    if (place !== 'constructor' && place !== 'ngOnInit' && place !== 'field')
        conditions.push('registration is conditional on the containing call');
    const chain = operators.map(item => item.api?.name);
    if (chain.includes('takeUntilDestroyed'))
        conditions.push('lifetime ends on DestroyRef destruction');
    else if (chain.includes('take'))
        conditions.push('lifetime may end at the take limit');
    else
        conditions.push('unsubscribe or owner destruction is not proven');
...
function registrationIndex(context, owner) {
    const t = context.toolchain.typescript;
    const output = [];
    const visit = (node, place) => {
            ...
            if (record) {
                for (let parent = node.parent; parent && parent !== owner.node; parent = parent.parent) {
                    if (t.isIfStatement(parent))
                        record.conditions.push(`registration requires ${parent.expression.getText()}`);
    ...
    if (hasUnsubscribe)
        for (const record of output)
            record.conditions.push('an unsubscribe call exists; its receiver and execution path determine lifetime');
    return output;
}
function ownedMethod(context, owner, call) {
    const t = context.toolchain.typescript;
...
export function traceOperation(context, owner, methodName) {
    ...
    const add = (kind, source, target, node, path, timing, conditions = [], detail = null) => {
        if (steps.length >= maxSteps)
            return;
        steps.push({ kind, source, target, location: location(context, node), path: [...path], timing,
            conditions: [...conditions], detail });
    };
    const visit = (node, path, depth, conditions) => {
        if (steps.length >= maxSteps)
            return;
        if (depth >= maxDepth) {
            add('boundary', methodName, 'depth limit', node, path, 'unknown', conditions);
            return;
        }
        if (t.isIfStatement(node)) {
            visit(node.thenStatement, path, depth, [...conditions, `if ${node.expression.getText()}`]);
            if (node.elseStatement)
                visit(node.elseStatement, path, depth, [...conditions, `else of ${node.expression.getText()}`]);
            return;
        }
        ...
        if (t.isCallExpression(node)) {
            ...
            if (t.isPropertyAccessExpression(node.expression) && node.expression.name.text === 'pipe') {
                ...
                if (sourceApi?.family === 'rxjs' && ['of', 'from', 'forkJoin', 'timer'].includes(sourceApi.name)) {
                    ...
                    add('reactive-link', methodName, sourceApi.name, sourceCall, [...nextPath, location(context, sourceCall)], semantics.timing, [...conditions, ...semantics.conditions], semantics.mode);
                }
                ...
                for (const [index, operator] of operators.entries()) {
                    ...
                    if (operator.boundary || !operator.semantics) {
                        add('boundary', previous, operator.name, operatorCall ?? node, [...nextPath, operator.location], 'unknown', conditions, operator.boundary);
                        break;
                    }
                    add('reactive-link', previous, operator.name, operatorCall ?? node, [...nextPath, operator.location], operator.semantics.timing, [...conditions, ...operator.semantics.conditions], operator.semantics.mode);
            ...
            if (method) {
                ...
                if (active.has(method))
                    add('boundary', methodName, method.name.getText(), node, nextPath, 'unknown', [...conditions, 'recursive call on the current branch']);
                else if (method.body) {
                    active.add(method);
            ...
            if (t.isPropertyAccessExpression(expr)) {
                ...
                if (action === 'emit') {
                    ...
                    if ([...owner.outputs.values()].includes(target)) {
                        ...
                        add('output-emit', methodName, target, node, nextPath, asyncEmitter ? 'async-output' : 'output', [...conditions, ...(asyncEmitter ? ['EventEmitter(true) schedules delivery asynchronously'] : [])], node.arguments[0]?.getText() ?? null);
                        return;
                    }
                    if (angularEmitter) {
                        add('boundary', methodName, target, node, nextPath, 'unknown', conditions, 'EventEmitter instance is not an exposed component output');
                        return;
                    }
                }
                if (action === 'set' || action === 'update') {
                    ...
                    if (isSignal) {
                        add('state-write', methodName, target, node, nextPath, 'sync', conditions, `${action}(${node.arguments[0]?.getText() ?? ''})`);
                        return;
                    }
                }
                if (action === 'next') {
                    ...
                    if (isSubject) {
                        add('state-write', methodName, target, node, nextPath, 'sync', conditions, `Subject.next(${node.arguments[0]?.getText() ?? ''})`);
                        for (const registration of registrations.filter(item => item.source === target && item.active)) {
                            ...
                            for (const [index, operator] of registration.operators.entries()) {
                                ...
                                for (const [callbackIndex, callback] of callbackArgs.entries()) {
                                    ...
                                    if (t.isArrowFunction(callback) || t.isFunctionExpression(callback))
                                    ...
                                    else if (t.isObjectLiteralExpression(callback))
                                        for (const property of callback.properties) {
                                            ...
                                            if (t.isArrowFunction(body) || t.isFunctionExpression(body))
                                                visit(body.body, [...registrationPath, operator.location], depth + 1, [...callbackConditions, `${property.name.getText()} notification`]);
                                        }
                                }
                            }
            ...
            if (api && ['computed', 'effect'].includes(api.name)) {
                add('reactive-link', methodName, api.name, node, nextPath, api.name === 'effect' ? 'change-detection' : 'sync', [...conditions, api.name === 'effect' ? 'effect callback runs on initial registration and tracked dependency changes' :
            ...
            if (!api && t.isPropertyAccessExpression(expr) && expr.expression.kind === t.SyntaxKind.ThisKeyword)
                add('boundary', methodName, expr.name.text, node, nextPath, 'unknown', conditions, 'member call is inherited, dynamic, or outside the local operation scope');
        }
        t.forEachChild(node, child => visit(child, path, depth, conditions));
    };
    active.add(root);
    if (root.body)
        visit(root.body, [location(context, root)], 0, []);
    if (steps.length >= maxSteps)
        diagnostics.push(`Operation trace reached ${maxSteps} steps`);
