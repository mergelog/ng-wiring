...
export function traceStoreDispatch(context, graph, owner, methodName, layers = [], options = {}) {
        ...
        const visit = (node, localConditions, level) => {
            ...
            if (t.isCallExpression(node) && t.isPropertyAccessExpression(node.expression)) {
                ...
                if (callee.expression.kind === t.SyntaxKind.ThisKeyword) {
                    const declaration = method.parent;
                    const target = t.isClassDeclaration(declaration) ? declaration.members.find(member => t.isMethodDeclaration(member) && member.name.getText() === callee.name.text) : null;
    ...
    if (root && t.isMethodDeclaration(root))
        visitMethod(root, owner.id, [location(context, root)], [], 0, root, []);
    else
                ...
                for (const relation of bindings.relations.filter(item => item.kind === 'output-subscription' &&
                    ...
                    for (const name of methods) {
                        ...
                        if (!parentMethod || !t.isMethodDeclaration(parentMethod)) {
                            add('boundary', owner.id, name, root, emitted.path, [...emitted.conditions, ...relation.conditions], 'output handler method is unresolved');
                            continue;
                        }
