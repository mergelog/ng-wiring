import { edgeContracts, edgeKinds } from '../model/types.js';
...
export const sentenceSlots = (kind) => [...new Set([...(sentenceTemplates[kind] ?? '').matchAll(slotPattern)].map(match => match[1]))];
/**
 ...
 */
export function displayGroupProblems() {
    const problems = [];
    for (const group of displayGroups) {
        if (edgeKinds.includes(group))
            problems.push(`Display group ${group} is also an edge kind`);
        if (group in sentenceTemplates)
            problems.push(`Display group ${group} has a sentence of its own`);
    }
    for (const kind of edgeKinds)
        if (!displayGroupOf[kind])
            problems.push(`Edge kind ${kind} has no display group`);
    return problems;
}
/**
 ...
 */
export function renderSentence(kind, details) {
    const template = sentenceTemplates[kind];
    if (!template || !edgeContracts[kind])
        throw new RenderError(`No sentence is defined for edge kind ${kind}`);
    const unresolved = [];
    const text = template.replace(slotPattern, (_match, key) => {
        const field = details[key];
        if (!field)
            throw new RenderError(`Edge kind ${kind} is missing the ${key} detail`);
 ...
 */
export function renderCondition(conditions, id, depth = 0) {
    if (id === null)
        return null;
    if (depth >= conditionDepthLimit)
        return '…（条件木の表示深さ上限）';
    const condition = conditions.get(id);
    if (!condition)
        return `未知の条件参照 ${escapeInline(id)}`;
