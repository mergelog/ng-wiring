...
export function pathHash(ownerId, file) {
 ...
 */
export function encodeName(raw) {
    let encoded = '';
    for (const char of raw.normalize('NFC')) {
        if (keptInName.test(char)) {
            encoded += char;
            continue;
        }
...
export function shortenName(encoded, raw) {
    if (encoded.length <= nameLimit)
        return encoded;
    let taken = 0;
    while (taken < encoded.length) {
        const width = encoded[taken] === '%' ? 3 : 1;
        if (taken + width > nameKept)
            break;
        taken += width;
    }
