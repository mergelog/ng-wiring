...
import { analyzeWorkspace, contextOf } from './analysis.js';
...
export function createBackend(input) {
    ...
    const keyOf = (options) => JSON.stringify([options.target, options.project ?? null, options.tsconfig ?? null]);
    const analysisFor = async (options, signal) => {
    ...
    return {
        ...
        async write(candidate, options, signal) {
            const analysis = await analysisFor(options, signal);
            const found = analysis.candidates.find(item => item.candidate.id === candidate.id);
            if (!found)
                throw new UsageError(`Candidate ${candidate.id} is outside the discovered candidates`);
