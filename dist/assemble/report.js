...
const chainedHandlerMethod = (handler, element, owner, context) => {
    const ng = context.toolchain.angularCompiler;
    const t = context.toolchain.typescript;
    const expression = handler.ast ?? handler;
    if (!(expression instanceof ng.Chain))
        return null;
    const methods = new Set();
    for (const part of expression.expressions) {
        if (!(part instanceof ng.Call) || !(part.receiver instanceof ng.PropertyRead))
            continue;
        const receiver = part.receiver;
        const implicit = receiver.receiver instanceof ng.ImplicitReceiver;
        if (!implicit && !(receiver.receiver instanceof ng.ThisReceiver))
            continue;
        if (implicit && element.lexical.has(receiver.name))
            continue;
        const method = classMethod(context, owner.node, receiver.name);
        if (!method || method.modifiers?.some(modifier => modifier.kind === t.SyntaxKind.PrivateKeyword))
            continue;
...
function addOperations(input) {
    const { analysis, builder, evidence, conditions, connect, declarationNode, relative, spanOf, targetElement, targetNodeId, targetKind, placed, viewPath, dynamicCallers: suppliedCallers, options, operationIds, problems, resolvedGaps, stores } = input;
    ...
    const bootstrap = routes.bootstraps.find(item => placed.some(step => step.step.relation === 'bootstrap' && step.step.ownerId === item.id))
        ?? routes.bootstraps[0];
    const displayRoute = placed
        .map(item => item.step.routeRef && routes.byId.get(item.step.routeRef.occurrenceId))
        .find((item) => !!item);
    const requiredMode = viewPath.steps.flatMap(item => [...(item.displayCondition ?? '').matchAll(/\bmode\s*===\s*['"]([^'"]+)['"]/g)].map(match => match[1]))[0];
    const fixedDialogMode = (call) => {
            ...
            if (t.isCallExpression(node) && t.isPropertyAccessExpression(node.expression) && node.expression.name.text === 'open') {
                ...
                if (config && t.isObjectLiteralExpression(config)) {
                    ...
                    if (data && t.isPropertyAssignment(data) && t.isObjectLiteralExpression(data.initializer)) {
                        ...
                        if (field && t.isPropertyAssignment(field) && t.isStringLiteralLike(field.initializer))
                            mode = field.initializer.text;
                    }
                }
            }
        ...
        const findResult = (node) => {
            if (t.isCallExpression(node) && t.isPropertyAccessExpression(node.expression) &&
                ...
                if (sameRef) {
                    ...
                    while (ancestor && ancestor !== method && !(t.isCallExpression(ancestor) && t.isPropertyAccessExpression(ancestor.expression) &&
    ...
    const defaultDialogInjector = (call) => {
        if (call.kind !== 'dialog')
            return false;
        const source = context.program.getSourceFile(path.resolve(context.workspaceRoot, call.file));
        if (!source)
        ...
        if (!found)
            return false;
        const config = found.arguments[1];
        if (!config)
            return true;
        if (!t.isObjectLiteralExpression(config))
            return false;
    ...
    const commonCallerProviders = !routeOccurrence && allCallerRoutesKnown && selectedBootstrap &&
        uniqueRoutes.length > 1 ? uniqueRoutes.map(route => storeInputsForSelection(context, catalog, routes, selectedBootstrap, route)) : [];
    const expandProviders = (nodes, visited = new Set()) => {
        const result = [];
    ...
    const providerKey = (node) => `${node.getSourceFile().fileName}:${node.getStart()}`;
    const sharedRouteProviders = commonCallerProviders.length
        ? expandProviders(commonCallerProviders[0]?.routeProviders ?? []).filter(node => commonCallerProviders.every(input => expandProviders(input.routeProviders ?? []).some(other => providerKey(other) === providerKey(node)))) : [];
    ...
    if (dynamicCallers.length) {
        ...
        if (!routeOccurrence)
            builder.diagnostic({ code: 'dynamic-route-context', severity: 'info',
    ...
    if (commonRouteContext)
        storeInputs.routeProviders = sharedRouteProviders;
    ...
    for (const link of signals.links.filter(item => item.capability === 'angular/toSignal')) {
        ...
        materialize([{ kind: 'reactive-link',
                ...
                details: completeDetails('reactive-link', { source: detail(sourceLabel),
                    consumer: detail(targetLabel), operator: detail(link.capability), scheduling: detail('subscription') }) }], background);
        if (link.to)
            backgroundKeys.push({ ownerId: at.classId, member: link.to, node: target });
    }
    ...
    const reportListeners = resolution.listeners.filter(listener => !resolution.outputSubscriptions.some(output => listener.eventSource === 'unknown-dom' && output.listenerElement === listener.listenerElement &&
    ...
    for (const listener of [...reportListeners, ...resolution.outputSubscriptions]) {
        ...
        const eventPosition = listener.listenerElement?.eventSpans.findIndex((span, index) => !!span && span.file === listener.span?.file && span.start === listener.span?.start &&
