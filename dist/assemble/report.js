...
function addOperations(input) {
    ...
    for (const listener of [...resolution.listeners, ...resolution.outputSubscriptions]) {
        ...
        if (method && owner) {
            ...
            const entered = new Set([method, ...storeTrace.steps.filter(step => step.kind === 'call')
...
function addDisplayReads(input) {
    const { analysis, builder, evidence, connect, declarationNode, spanOf, keys, placed, scope, storeMemberFor } = input;
    if (!keys.length)
        return;
