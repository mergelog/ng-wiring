...
候補一覧は番号・分類に加えて、ルートパターン、root から対象までのコンポーネント経路、使用箇所のファイルと行、対象テンプレートのファイルと offset、完全な候補 ID を表示する。同じ静的要素が複数の画面や親経路から使われる場合、その違いを番号選択前に読めるようにする。offset は行番号として表示しない。
