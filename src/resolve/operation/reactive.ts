...
export interface ApiIdentity { family: ApiFamily; name: string }
...
export function importedApi(context: AnalysisContext, node: ts.Node): ApiIdentity | null {
  ...
  if (files.some(file => file.includes('/node_modules/@angular/core/')) && angularApis.has(name)) return { family: 'angular', name };
...
export interface OperatorSemantics { timing: 'sync' | 'timer' | 'microtask' | 'subscription' | 'unknown'; conditions: string[]; mode: string }
const known: Record<string, OperatorSemantics> = {
...
export function operatorSemantics(name: string): OperatorSemantics | null { return known[name] ?? null; }

export interface OperatorRecord { name: string; api: ApiIdentity | null; location: string; semantics: OperatorSemantics | null;
  boundary: string | null }
export function inspectPipe(context: AnalysisContext, call: ts.CallExpression): OperatorRecord[] {
  const t = context.toolchain.typescript;
  if (!t.isPropertyAccessExpression(call.expression) || call.expression.name.text !== 'pipe') return [];
  ...
  return call.arguments.map(argument => {
    if (!t.isCallExpression(argument)) return { name: argument.getText(), api: null, location: location(context, argument),
...
export interface AsyncPipeConsumer { expression: string; span: Span; status: 'resolved' | 'boundary';
  conditions: string[]; pipeId: string | null }
/** AsyncPipe is a template subscription, with view destruction as its lifetime. */
export function inspectAsyncPipe(element: IndexedElement, context: AnalysisContext, catalog: Catalog): AsyncPipeConsumer[] {
  ...
  const result: AsyncPipeConsumer[] = [];
  const visitor = new class extends ng.RecursiveAstVisitor {
    override visitPipe(ast: InstanceType<typeof ng.BindingPipe>, contextValue: unknown): unknown {
      ...
      if (ast.name === 'async') result.push({ expression: inlineExpression || '(template expression)', span: element.span,
  ...
  for (const child of element.node.children) if (child instanceof ng.TmplAstBoundText) child.value.visit(visitor);
  return result;
}
export function location(context: AnalysisContext, node: ts.Node): string {
