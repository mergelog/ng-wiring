...
export function traceStoreDispatch(context: AnalysisContext, graph: StoreGraph, owner: Declaration,
  ...
  const actionFor = (expression: ts.Expression): StoreAction | undefined => {
    if (t.isObjectLiteralExpression(expression)) {
      const typeProperty = expression.properties.find((property): property is ts.PropertyAssignment =>
      ...
      if (actionType) return graph.actions.find(action => action.type === actionType);
      return undefined;
    }
    ...
    if (t.isPropertyAccessExpression(callee) && t.isIdentifier(callee.expression)) {
      ...
      if (group && t.isVariableDeclaration(group) && group.initializer && t.isCallExpression(group.initializer)) {
        ...
        if (isActionGroup) id = `${tokenId(context, group.name)}:${callee.name.text}`;
      }
    }
  ...
  const signalStoreMethod = (implementation: string, name: string): StoreMethodNode | null => {
    ...
    const methodInStoreCall = (call: ts.CallExpression): StoreMethodNode | null => {
      ...
      const findInFeature = (expression: ts.Expression, depth = 0): StoreMethodNode | null => {
        ...
        if (api === 'withMethods') {
          ...
          if (!factory || (!t.isArrowFunction(factory) && !t.isFunctionExpression(factory))) return null;
        ...
        if (api === 'withFeature') {
          ...
          if (factory && (t.isArrowFunction(factory) || t.isFunctionExpression(factory))) {
            const returned = t.isBlock(factory.body)
              ? factory.body.statements.find(t.isReturnStatement)?.expression : factory.body;
            if (returned) return findInFeature(returned, depth + 1);
          }
          return null;
        }
        ...
        const returned = body && (t.isBlock(body)
          ? body.statements.find(t.isReturnStatement)?.expression : body);
        if (returned && returned !== feature) return findInFeature(returned, depth + 1);
        return null;
      };
    ...
    for (const file of context.sourceFiles) {
      ...
      if (!source) continue;
      for (const statement of source.statements) {
        if (t.isVariableStatement(statement)) for (const declaration of statement.declarationList.declarations) {
          if (tokenId(context,declaration.name) !== implementation || !declaration.initializer) continue;
        ...
        if (t.isClassDeclaration(statement) && statement.name && tokenId(context, statement.name) === implementation)
          for (const clause of statement.heritageClauses ?? []) if (clause.token === t.SyntaxKind.ExtendsKeyword)
    ...
    const visit = (node: ts.Node, localConditions: string[], level: number): void => {
        ...
        if (callee.name.text === 'subscribe' && options.afterClosedLocation) {
          ...
          if (afterClosed) {
            const filterConditions = (pipeline?.arguments ?? []).flatMap(operator => {
              if (!t.isCallExpression(operator)) return [];
            ...
            add('reactive-link', receiver, 'MatDialogRef.afterClosed', afterClosed, nextPath,
      ...
      if (t.isCallExpression(node) && t.isIdentifier(node.expression)) {
        ...
        if (api?.family === 'rxjs' && ['forkJoin', 'lastValueFrom', 'firstValueFrom', 'of', 'from'].includes(api.name)) {
          ...
          if (semantics) add('reactive-link', methodName, api.name, node, [...path, location(context,node)],
            [...localConditions, ...semantics.conditions], semantics.mode);
        }
