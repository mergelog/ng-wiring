  ...
  const configProviders = (expression: ts.Expression, active = new Set<ts.Node>()): ts.Expression[] => {
    ...
    if (t.isCallExpression(node)) {
      ...
      const returned = body && t.isBlock(body)
        ...
        : body;
      if (returned) return configProviders(returned, active);
    }
    return [];
  };
...
export function analyzeStore(context: AnalysisContext, catalog: Catalog, inputs: StoreInputs): StoreGraph {
    ...
    const visit = (node: ts.Node): void => {
      if (t.isVariableDeclaration(node) && node.initializer && t.isCallExpression(node.initializer)) {
        ...
        if (callName(context, call, 'createActionGroup', 'store') && call.arguments[0] &&
          ...
          if (sourceName && events && t.isObjectLiteralExpression(events)) for (const event of events.properties) {
            if (!t.isPropertyAssignment(event) || !t.isStringLiteralLike(event.name)) continue;
            const member = event.name.text.toLowerCase().replace(/[^a-z0-9]+([a-z0-9])/g,
        ...
        if (callName(context, call, 'createFeature', 'store') && call.arguments[0] &&
          ...
          const config = definition(context,call.arguments[0]) as ts.ObjectLiteralExpression;
            ...
            if (field) {
              ...
              while (parent && parent !== owner.node) {
                if (t.isCallExpression(parent) && t.isPropertyAccessExpression(parent.expression) &&
                  parent.expression.name.text === 'subscribe') { subscribed = true; break; }
                if (t.isPropertyDeclaration(parent) || t.isPropertyAssignment(parent)) break;
  ...
  for (const call of effectCalls) {
    ...
    const actionReturns = (value: ts.Expression, branch: string[] = []): { id: string; branch: string[] }[] => {
      const result = definition(context,value);
      if (t.isConditionalExpression(result)) return [
      ...
      if (t.isArrayLiteralExpression(result)) return result.elements.flatMap(element =>
        actionReturns(t.isSpreadElement(element) ? element.expression : element, branch));
      if (!t.isCallExpression(result)) return [];
      const id = actionId(context,result);
      if (actions.some(action => action.id === id)) return [{ id, branch }];
      const callee = t.isPropertyAccessExpression(result.expression) ? result.expression.name : result.expression;
      if (importedApi(context,callee)?.name === 'of') return result.arguments.flatMap(arg => actionReturns(arg, branch));
      return [];
    };
