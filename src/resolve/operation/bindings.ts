...
export interface BindingResolution { relations: BindingRelation[]; diagnostics: string[] }

function importSymbol(context: AnalysisContext, name: ts.Identifier, pkg: string): string | null {
...
function memberNode(context: AnalysisContext, declaration: Declaration, name: string): ts.ClassElement | null {
...
function details(context: AnalysisContext, declaration: Declaration | undefined, member: string): {
  model: boolean; defaultValue: string | null; required: boolean; transform: string | null; asyncOutput: boolean;
} {
  const t = context.toolchain.typescript;
  const node = declaration && memberNode(context, declaration, member);
  if (!node) return { model: false, defaultValue: null, required: false, transform: null, asyncOutput: false };
...
function mutationConditions(context: AnalysisContext, declaration: Declaration, member: string): string[] {
  const t = context.toolchain.typescript;
  const result: string[] = [];
  ...
  for (const part of declaration.node.members) visit(part);
  return [...new Set(result)];
}
function pipeNames(context: AnalysisContext, ast: import('@angular/compiler').AST): string[] {
  const ng = context.toolchain.angularCompiler;
  const names: string[] = [];
  const visitor = new class extends ng.RecursiveAstVisitor {
    override visitPipe(node: InstanceType<typeof ng.BindingPipe>, contextValue: unknown): unknown {
...
function formAccessor(element: IndexedElement): { target: string | null; reason: string } {
  ...
  if (candidates.length === 1 && !custom.length) return { target: candidates[0]!.id,
...
export function resolveElementBindings(element: IndexedElement, context: AnalysisContext, catalog: Catalog): BindingResolution {
  const relations: BindingRelation[] = [];
  ...
  for (const [alias, targets] of element.appliedInputs) {
    ...
    if (bound || hasAttribute) usedInputs.add(alias);
    for (const target of targets) {
      ...
      if (bound || hasAttribute) {
        ...
        if (info.transform) conditions.push(`input transform ${info.transform} must be evaluated`);
        ...
        if (!element.component && element.gaps.some(g => g.includes('Ambiguous component'))) conditions.push('component target is ambiguous');
        ...
        if (declaration && memberNode(context, declaration, 'ngOnChanges')) relations.push({ kind: 'input-change', alias, member: 'ngOnChanges',
