...
export interface UrlResolution { status: 'static' | 'partial' | 'unresolved'; text: string | null;
  segments: UrlSegment[]; reason: string | null }
...
export interface TypeUse { id: string; name: string; role: TypeRole; origin: 'type-argument' | 'declared' | 'inferred';
  source: string }
export interface HttpBranch { operator: string; location: string; effect: string }
...
export interface HttpInterceptor { id: string; name: string; kind: 'function' | 'class'; source: string | null }
export interface HttpEnvironment { registered: boolean; source: string | null; features: string[];
  interceptors: HttpInterceptor[]; conditions: string[]; gaps: string[] }
export interface HttpCatalog { requests: HttpRequestSite[]; environment: HttpEnvironment; diagnostics: string[] }

const methodByMember: Record<string, HttpMethod> = { get: 'GET', post: 'POST', put: 'PUT', patch: 'PATCH',
...
const branchEffects: Record<string, string> = {
...
export function httpBranchEffect(operator: string): string | null { return branchEffects[operator] ?? null; }
...
function moduleOf(context: AnalysisContext, symbol: ts.Symbol | undefined): string | null {
...
export function rxjsExport(context: AnalysisContext, node: ts.Node): string | null {
...
function fromHttpPackage(context: AnalysisContext, symbol: ts.Symbol | undefined): boolean {
  const module = moduleOf(context, symbol);
  return module === HTTP_MODULE || module === '@angular/common';
}
export function httpApiExport(context: AnalysisContext, node: ts.Node): string | null {
  const symbol = symbolOf(context, node);
...
function httpClientReceiver(context: AnalysisContext, receiver: ts.Expression): boolean {
...
function isGlobalFetch(context: AnalysisContext, node: ts.Expression): boolean {
  const t = context.toolchain.typescript;
  ...
  if (!t.isIdentifier(node) || node.text !== 'fetch') return false;
...
function relativeFile(context: AnalysisContext, file: string): string {
  const root = `${slash(context.workspaceRoot)}/`;
  const name = slash(file);
  return name.startsWith(root) ? name.slice(root.length) : name;
}
function enclosing(context: AnalysisContext, node: ts.Node): { owner: string | null; member: string | null } {
  const t = context.toolchain.typescript;
  ...
  for (let cursor: ts.Node | undefined = node.parent; cursor; cursor = cursor.parent) {
    if (!member) {
      if (t.isMethodDeclaration(cursor) || t.isFunctionDeclaration(cursor) || t.isGetAccessorDeclaration(cursor))
        ...
        cursor.name) member = cursor.name.getText();
    }
...
export function resolveUrl(context: AnalysisContext, expression: ts.Expression | undefined): UrlResolution {
  const t = context.toolchain.typescript;
  if (!expression) return { status: 'unresolved', text: null, segments: [],
    reason: 'the call has no URL argument in the resolved overload' };
  const segments: UrlSegment[] = [];
  const unresolved: string[] = [];
  let depth = 0;
  const literal = (text: string): void => { if (text) segments.push({ kind: 'literal', text }); };
  const walk = (input: ts.Expression): void => {
    if (depth++ > 64) { unresolved.push('URL expression depth limit'); return; }
    const node = unwrap(t, input);
    if (t.isStringLiteralLike(node)) { literal(node.text); return; }
    ...
    if (evaluated.known && typeof evaluated.value === 'string') { literal(evaluated.value); return; }
  ...
  if (!unresolved.length) return { status: 'static', text, segments, reason: null };
...
function declaredInWorkspace(context: AnalysisContext, declaration: ts.Declaration): boolean {
...
export function collectTypes(context: AnalysisContext, type: ts.Type, role: TypeRole,
  origin: TypeUse['origin'], into: Map<string, TypeUse>, depth = 0): void {
  const t = context.toolchain.typescript;
  if (depth > 8) return;
  if (type.isUnionOrIntersection()) {
    for (const part of type.types) collectTypes(context, part, role, origin, into, depth + 1);
    return;
  }
  for (const symbol of [type.aliasSymbol, type.getSymbol()]) {
    ...
    if (!symbol || !declaration || !declaredInWorkspace(context, declaration)) continue;
  ...
  for (const argument of args) collectTypes(context, argument, role, origin, into, depth + 1);
}

function observeOption(context: AnalysisContext, options: ts.Expression | undefined): string | null {
  const t = context.toolchain.typescript;
  if (!options) return null;
  const node = unwrap(t, options);
  if (!t.isObjectLiteralExpression(node)) return null;
...
function pipeBranches(context: AnalysisContext, call: ts.CallExpression,
  branches: HttpBranch[], gaps: string[]): void {
  const t = context.toolchain.typescript;
  ...
  while (t.isPropertyAccessExpression(cursor.parent) && cursor.parent.expression === cursor &&
    ...
    for (const argument of outer.arguments) {
      ...
      if (effect) branches.push({ operator: name, location: location(context, argument), effect });
...
export function requestSiteAt(context: AnalysisContext, call: ts.CallExpression): HttpRequestSite | null {
  const t = context.toolchain.typescript;
  const conditions: string[] = ['the number of network calls is not proven here; interceptors, caching, retry and sharing can change it'];
  const gaps: string[] = [];
  const types = new Map<string, TypeUse>();
  ...
  if (isGlobalFetch(context, call.expression)) {
    ...
    const literalMethod = methodValue && t.isStringLiteralLike(methodValue)
      ? methodValue.text.toUpperCase() as HttpMethod : null;
    const known = literalMethod && Object.values(methodByMember).includes(literalMethod);
    if (call.arguments[0]) collectTypes(context, context.checker.getTypeAtLocation(call.arguments[0]), 'url', 'inferred', types);
    if (init) collectTypes(context, context.checker.getTypeAtLocation(init), 'request-body', 'declared', types);
  ...
  let method: HttpMethod | null = methodByMember[member] ?? null;
  let methodReason: string | null = null;
  let urlArgument: ts.Expression | undefined = call.arguments[0];
  let bodyArgument: ts.Expression | undefined = bodyMembers.has(member) ? call.arguments[1] : undefined;
  let optionsArgument: ts.Expression | undefined = call.arguments[bodyMembers.has(member) ? 2 : 1];
  if (member === 'request') {
    ...
    const evaluated = methodExpression ? new StaticEvaluator(t, context.checker).evaluate(methodExpression)
      : { known: false as const, reason: 'missing method argument' };
    if (evaluated.known && typeof evaluated.value === 'string') {
      const upper = evaluated.value.toUpperCase() as HttpMethod;
      method = Object.values(methodByMember).includes(upper) ? upper : 'unknown';
      if (method === 'unknown') methodReason = `request method ${JSON.stringify(evaluated.value)} is not a known HTTP method`;
    } else {
      method = 'unknown';
  ...
  if (typeArgument) collectTypes(context, context.checker.getTypeFromTypeNode(typeArgument), 'response', 'type-argument', types);
  else {
    const returned = context.checker.getTypeAtLocation(call);
    collectTypes(context, returned, 'response', 'inferred', types);
  }
  if (bodyArgument) collectTypes(context, context.checker.getTypeAtLocation(bodyArgument), 'request-body', 'declared', types);
  if (urlArgument) collectTypes(context, context.checker.getTypeAtLocation(urlArgument), 'url', 'inferred', types);
  ...
  if (url.status !== 'static' && url.reason) conditions.push(url.reason);
  if (methodReason) gaps.push(`${methodReason} at ${location(context, call)}`);
  ...
  const declaredReturn = holder && 'type' in holder ? (holder as ts.SignatureDeclaration).type : undefined;
...
function interceptorsFrom(context: AnalysisContext, call: ts.CallExpression, environment: HttpEnvironment): void {
  const t = context.toolchain.typescript;
...
export function analyzeHttpEnvironment(context: AnalysisContext): HttpEnvironment {
  const t = context.toolchain.typescript;
  const environment: HttpEnvironment = { registered: false, source: null, features: [], interceptors: [],
    conditions: [], gaps: [] };
  const visit = (node: ts.Node): void => {
    if (t.isCallExpression(node)) {
      ...
      if (api === 'provideHttpClient') {
        ...
        for (const feature of node.arguments) {
          ...
          if (name === 'withInterceptors') interceptorsFrom(context, feature, environment);
    ...
    if (t.isObjectLiteralExpression(node)) {
      ...
      if (tokenName === 'HTTP_INTERCEPTORS') {
        ...
        const declaration = symbol?.valueDeclaration ?? symbol?.declarations?.[0];
        if (declaration && symbol) environment.interceptors.push({
        ...
        else environment.gaps.push(`HTTP_INTERCEPTORS provider at ${location(context, node)} has no resolved implementation`);
...
export function analyzeHttp(context: AnalysisContext): HttpCatalog {
  const t = context.toolchain.typescript;
  const requests: HttpRequestSite[] = [];
  const diagnostics: string[] = [];
  const visit = (node: ts.Node): void => {
    if (t.isCallExpression(node)) {
      const site = requestSiteAt(context, node);
      if (site) requests.push(site);
    }
    t.forEachChild(node, visit);
  };
  for (const file of context.sourceFiles) {
