...
const end = (kind: NodeKind, id: string, label = id): TracedEnd => ({ kind, id, label });
const known = (value: string | null | undefined, reason: string): DetailField =>
  value ? detail(value) : unresolvedDetail(reason);

/** §8 every required detail is present; one the layer did not report stays null with its reason. */
export function completeDetails(kind: EdgeKind, provided: Record<string, DetailField>): Record<string, DetailField> {
...
function traced(input: Omit<TracedEdge, 'details'> & { details: Record<string, DetailField> }): TracedEdge {
...
export function httpTraceEdges(trace: HttpTrace): TracedEdge[] {
  const sites = new Map(trace.flows.map(flow => [flow.request.id, flow] as const));
  const edges: TracedEdge[] = [];
  for (const step of trace.steps) {
    const common = { location: step.location, conditions: step.conditions, capability: null };
...
export function reactiveStepEdges(steps: readonly ReactiveStep[]): TracedEdge[] {
  const edges: TracedEdge[] = [];
...
function stateId(step: ReactiveStep): string {
  const state = step.state;
  if (!state) return step.kind === 'state-read' ? step.source : step.target;
