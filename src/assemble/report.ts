...
import { traceHttpFromEffect, traceHttpFromEventConsumer, traceHttpFromMethod,
  traceHttpFromStoreMethod } from '../resolve/operation/http-flow.js';
...
import { completeDetails, httpTraceEdges, operationTraceEdges, reactiveStepEdges, storeTraceEdges, type TracedEdge } from './steps.js';
...
function addOperations(input: OperationInput): void {
  ...
  const outputUses = new Map(placed.flatMap(item => item.element?.component
    ? [[item.element.component, item.element] as const] : []));
  const contextualInputs = new Map<string, string>();
  for (const item of placed) {
    if (item.element?.component !== selfOwner?.id) continue;
  ...
  const dialogTypeCallers = (): DynamicCaller[] => {
    if (!selfOwner || !t.isClassDeclaration(selfOwner.node) || !selfOwner.node.name) return [];
    const dialogSymbol = context.checker.getSymbolAtLocation(selfOwner.node.name);
    if (!dialogSymbol) return [];
    const found: DynamicCaller[] = [];
    for (const fileName of context.sourceFiles) {
      ...
      const visit = (node: ts.Node): void => {
        if (t.isCallExpression(node) && t.isPropertyAccessExpression(node.expression) &&
          ...
          const packageDialog = receiverType?.getName() === 'MatDialog' && receiverType.declarations?.some(item =>
            slash(item.getSourceFile().fileName).includes('/node_modules/@angular/material/'));
          ...
          if (packageDialog && componentId === selfOwner.id) {
            ...
            while (owner && !t.isClassDeclaration(owner)) owner = owner.parent;
            ...
            const findSubscription = (part: ts.Node): void => {
              if (subscribed || !method?.body) return;
              if (t.isCallExpression(part) && t.isPropertyAccessExpression(part.expression) &&
                ...
                if (sameRef) {
                  let ancestor: ts.Node | undefined = part.parent;
            ...
            if (method?.body) findSubscription(method.body);
  ...
  const dialogResultMethod = (call: DynamicCaller): { owner: Declaration; method: string; location: string } | null => {
    ...
    const delivered: ts.CallExpression[] = [];
    const findResult = (node: ts.Node): void => {
      if (t.isCallExpression(node) && t.isPropertyAccessExpression(node.expression) &&
        ...
        if (sameRef) {
          ...
          if (ancestor && ancestor !== method) delivered.push(node);
        }
      }
  ...
  const storeMemberFor = (ownerId: string, instanceId: string): string | null => {
  ...
  const materialize = (edges: readonly TracedEdge[], into: { nodes: Set<string>; edges: string[];
    ...
    for (const edge of edges) {
      ...
      const predicates = [...(into.conditions ?? []), ...edge.conditions].map(text =>
  ...
  for (const listener of [...reportListeners, ...resolution.outputSubscriptions]) {
    ...
    const predicates = listenerConditions.map(text =>
    ...
    if (method && owner) {
      ...
      for (const member of owner.node.members) {
        if (!t.isPropertyDeclaration(member) || !member.initializer || !t.isCallExpression(member.initializer) ||
          !t.isIdentifier(member.name)) continue;
        if (!owner.outputs.has(member.name.text)) continue;
      ...
      for (const patch of patchStates) {
        if (!patch.member || !calledStoreMethods.has(patch.member)) continue;
        const at = evidence.offsetOf(patch.location);
        if (!at) continue;
        const source = context.program.getSourceFile(path.resolve(context.workspaceRoot, at.file));
        if (!source) continue;
        let storeMethod: ts.MethodDeclaration | null = null;
        const locate = (node: ts.Node): void => {
          if (storeMethod) return;
          if (t.isMethodDeclaration(node) && node.name.getText() === patch.member &&
            node.getStart() <= at.offset && node.getEnd() >= at.offset) { storeMethod = node; return; }
          t.forEachChild(node, locate);
        };
        locate(source);
        if (!storeMethod) continue;
        const store = [...stores.declarations.values()].find(record =>
          record.members.some(member => member.name === patch.member && member.kind === 'method'));
        if (!store) continue;
        const storeHttp = traceHttpFromStoreMethod(context, httpCatalog, storeMethod, store.name,
          { catalog, store: storeGraph, methods, stores, layers });
      ...
      for (const consumed of storeTrace.steps.filter(step => step.kind === 'action-consume')) {
        ...
        const effectHttp = traceHttpFromEffect(context, httpCatalog, effect,
          { catalog, store: storeGraph, methods, stores, layers }, consumed.conditions);
      ...
      const dialogCloseResults = (): string[] => {
      ...
      for (const call of dynamicCallers) {
        ...
        if (passesFalseCloseValue) continue;
        const result = dialogResultMethod(call);
        if (!result) {
          if (closeResults.length) builder.diagnostic({ code: 'dialog-result-boundary', severity: 'info',
      ...
      const changedByOwner = new Map<string, Set<string>>();
      for (const step of storeTrace.steps.filter(item => item.kind === 'state-write' &&
        catalog.declarations.has(item.source))) {
        const members = changedByOwner.get(step.source) ?? new Set<string>();
        members.add(step.target);
        changedByOwner.set(step.source, members);
      }
      for (const [parentId, changed] of changedByOwner) for (const child of index.byOwner.get(parentId) ?? []) {
        if (!child.component || !changed.size) continue;
        ...
        for (const bound of bindings.filter(item => item.kind === 'input-binding' &&
          ...
          const gate = conditions.all(phase.map(expression =>
            conditions.predicate({ expression, scope: parentId, evidenceId: at })));
...
interface ReactiveWriteInput {
  ...
  catalog: ContextAnalysis['catalog'];
...
function addReactiveWrites(input: ReactiveWriteInput): DisplayKey[] {
  ...
  for (const write of signals.writes.filter(item => inside(item.location))) {
    ...
    for (const link of signals.links) {
      ...
      const derivedKind: NodeKind = link.capability === 'angular/toObservable' ? 'operation' : 'state';
      ...
      if (link.to && derivedKind === 'state') keys.push({ ownerId: source?.state.instance ?? null, member: link.to,
  ...
  for (const dispatch of eventGraph.dispatches.filter(item => inside(item.source))) {
    ...
    const eventInstanceFor = (storeId: string | null) => stores.instances.find(item => item.created &&
    ...
    const eventSteps = eventDeliverySteps(eventGraph, dispatch, delivery).map(step => {
      if (step.kind !== 'state-write' || !step.state) return step;
    ...
    for (const consumer of delivery.consumers.filter(item => item.kind === 'handler')) {
      const http = traceHttpFromEventConsumer(context, httpCatalog, consumer,
        { catalog, store: storeGraph, stores, layers }, [...delivery.conditions, ...consumer.conditions]);
    ...
    for (const consumer of delivery.consumers) for (const key of consumer.writes) {
      ...
      if (instance) keys.push({ ownerId: consumer.owner, member: key,
        storeId: instance.id,
  ...
  for (const instance of stores.instances.filter(item => sameOwnerId(ownerId, item.owner) && item.created)) {
    ...
    for (const patch of patchStates) {
      ...
      const storeMember = declaration.members.find(member =>
        member.name === patch.member && member.kind === 'method');
      ...
      if (!featureRange || !withinRange(patch.location, featureRange)) continue;
      ...
      for (const member of declaration.members.filter(item =>
        (item.kind === 'computed' || item.kind === 'linked-state') && item.dependencies.some(key => written.includes(key)))) {
