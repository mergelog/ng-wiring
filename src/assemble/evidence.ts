...
export class SourceEvidence {
  ...
  private read(file: string): string | undefined {
    ...
    if (this.texts.has(absolute)) return this.texts.get(absolute);
  ...
  span(span: Span | null | undefined, precision: Precision = 'exact', symbolId: string | null = null): string | null {
    if (!span) return null;
    return this.table.tryAdd({ file: span.file, start: span.start, end: span.end, precision, symbolId });
  }

  node(node: ts.Node, precision: Precision = 'exact', symbolId: string | null = null): string | null {
   ...
   */
  location(text: string, symbolId: string | null = null): string | null {
  ...
  offsetOf(text: string): { file: string; offset: number } | null {
    const match = /^(.+):(\d+):(\d+)$/.exec(text);
    if (!match) return null;
    const file = match[1]!;
    const body = this.read(file);
