...
export type StaticValue = null | boolean | number | string | StaticValue[] | { [key: string]: StaticValue };
export type Evaluation = { known: true; value: StaticValue } | { known: false; reason: string };
...
export class StaticEvaluator {
  ...
  constructor(private readonly tsApi: typeof ts, private readonly checker: ts.TypeChecker,
    private readonly maxExpansions = 10_000, private readonly maxDepth = 64) {}
  ...
  private unknown(reason: string): Evaluation { return { known: false, reason }; }

  private visit(node: ts.Node, depth: number): Evaluation {
    const t = this.tsApi;
    if (++this.expansions > this.maxExpansions) return this.unknown('Static evaluation expansion limit');
    if (depth > this.maxDepth) return this.unknown('Static evaluation depth limit');
    if (this.active.has(node)) return this.unknown('Static evaluation cycle');
    ...
    try {
      ...
      if (t.isStringLiteralLike(node) || t.isNoSubstitutionTemplateLiteral(node)) return { known: true, value: node.text };
      if (t.isNumericLiteral(node)) return { known: true, value: Number(node.text) };
      if (node.kind === t.SyntaxKind.TrueKeyword) return { known: true, value: true };
      if (node.kind === t.SyntaxKind.FalseKeyword) return { known: true, value: false };
      if (node.kind === t.SyntaxKind.NullKeyword) return { known: true, value: null };
      ...
      if (t.isIdentifier(node)) {
        ...
        if (!symbol) return this.unknown(`Unresolved identifier ${node.text}`);
        if (symbol.flags & t.SymbolFlags.Alias) symbol = this.checker.getAliasedSymbol(symbol);
        const declaration = symbol.valueDeclaration ?? symbol.declarations?.[0];
        if (declaration && t.isVariableDeclaration(declaration) && declaration.initializer) {
          const list = declaration.parent;
          if (!t.isVariableDeclarationList(list) || !(list.flags & t.NodeFlags.Const)) return this.unknown(`Mutable identifier ${node.text}`);
      ...
      if (t.isArrayLiteralExpression(node)) {
        ...
        for (const item of node.elements) {
          ...
          if (t.isSpreadElement(item)) {
            if (!Array.isArray(value.value)) return this.unknown('Array spread is not an array');
      ...
      if (t.isObjectLiteralExpression(node)) {
        ...
        for (const property of node.properties) {
          if (t.isSpreadAssignment(property)) {
            ...
            if (!spread.known || !spread.value || typeof spread.value !== 'object' || Array.isArray(spread.value)) return this.unknown('Unknown object spread');
            ...
            if (key === undefined) return this.unknown('Computed object property');
          ...
          } else return this.unknown('Getter, method or accessor is not evaluated');
        }
        return { known: true, value };
      }
      if (t.isPropertyAccessExpression(node) || t.isElementAccessExpression(node)) {
        const owner = this.visit(node.expression, depth + 1);
        if (!owner.known || owner.value === null || typeof owner.value !== 'object') return this.unknown('Unknown property owner');
        ...
        if (name === undefined || !(name in owner.value)) return this.unknown(`Unknown property ${name ?? ''}`);
        return { known: true, value: (owner.value as Record<string, StaticValue>)[name]! };
      }
      if (t.isConditionalExpression(node)) {
        const condition = this.visit(node.condition, depth + 1);
        if (!condition.known || typeof condition.value !== 'boolean') return this.unknown('Unknown conditional test');
      ...
      if (t.isCallExpression(node) && node.arguments.length === 0 && t.isIdentifier(node.expression)) {
        ...
        if (symbol?.flags && symbol.flags & t.SymbolFlags.Alias) symbol = this.checker.getAliasedSymbol(symbol);
        const declaration = symbol?.valueDeclaration ?? symbol?.declarations?.[0];
        ...
        if (functionNode && (t.isArrowFunction(functionNode) || t.isFunctionExpression(functionNode) || t.isFunctionDeclaration(functionNode)) &&
          ...
          if (!t.isBlock(body)) return this.visit(body, depth + 1);
