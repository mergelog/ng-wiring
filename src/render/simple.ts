...
function dataRows(chain: Chain, edges: Map<string, ModelEdge>, nodes: Map<string, ModelNode>,
  evidence: Map<string, Evidence>, sources: Sources): Row[] {
  const result: Row[] = [];
  ...
  if (listener) push(listener, 'h', 'D', `(${first.event}) ${value(listener, 'handler')}`);
  ...
  if (!chain.request) return result;
...
export function renderSimple(input: RenderInput & { belowData?: boolean }): RenderResult {
  ...
  if (!primary?.request) out.push('- 通信: この探索範囲では未検出');
  out.push('', '## 凡例', '');
