...
export function encodeLinkPath(value: string): string {
  let encoded = '';
  for (const char of value) {
    if (safeInLink.test(char)) { encoded += char; continue; }
 ...
 */
export function relativeLinkTarget(fromDir: string, target: string, platform: path.PlatformPath = path): string | null {
  const relative = platform.relative(fromDir, target);
  if (!relative || platform.isAbsolute(relative)) return null;
