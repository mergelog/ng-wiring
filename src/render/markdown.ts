...
import type { CoverageGap, ModelPath, WiringReport } from '../model/types.js';
...
export interface RenderResult { text: string; edgeIds: string[]; problems: string[] }

const number = (index: number): string => String(index + 1).padStart(2, '0');
const listOr = (values: readonly string[], empty: string): string =>
  values.length ? values.map(escapeInline).join(', ') : empty;
...
export function renderMarkdown(input: RenderInput): RenderResult {
  ...
  const link = (id: string): string => {
    ...
    if (!item) { problems.push(`Evidence ${id} is missing from the report`); return `（根拠 ${escapeInline(id)} は記録にない）`; }
  ...
  const links = (ids: readonly string[]): string => ids.length ? ids.map(link).join(' , ') : '根拠なし';

  const labelOf = (id: string): { name: string; file: string | null } => {
    const node = nodes.get(id);
    if (!node) { problems.push(`Node ${id} is missing from the report`); return { name: id, file: null }; }
    ...
    if (explicit) return { name: explicit, file };
    if (symbol) { const hash = symbol.lastIndexOf('#'); return { name: hash >= 0 ? symbol.slice(hash + 1) : symbol, file }; }
  ...
  for (const item of report.paths) for (const id of item.occurrenceIds) if (!sectionIds.includes(id)) sectionIds.push(id);
  const sectionIndex = new Map(sectionIds.map((id, index) => [id, index] as const));
  const labels = new Map(sectionIds.map(id => [id, labelOf(id)] as const));
  const filesByName = new Map<string, Set<string>>();
  for (const label of labels.values()) {
    const known = filesByName.get(label.name) ?? new Set<string>();
    if (label.file) known.add(label.file);
  ...
  const edgeLine = (id: string): string => {
    ...
    if (!edge) { problems.push(`Edge ${id} is missing from the report`); return `- 未知の辺 ${escapeInline(id)}`; }
    rendered.add(id);
  ...
  const sectionEdges = new Map<string, string[]>();
  const pathOnlyEdges = new Set<string>();
  for (const edge of report.edges) {
    if (!inPaths.has(edge.id) || excluded.has(edge.id)) continue;
    const owner = sectionIndex.has(edge.to) ? edge.to : sectionIndex.has(edge.from) ? edge.from : null;
    if (owner === null) { pathOnlyEdges.add(edge.id); continue; }
    const list = sectionEdges.get(owner);
    if (list) list.push(edge.id); else sectionEdges.set(owner, [edge.id]);
  }
  ...
  if (report.coverage.reasons.length) {
    out.push('- 重要な未解決理由:');
    for (const reason of report.coverage.reasons.slice(0, 10)) out.push(`  - ${escapeInline(reason)}`);
    if (report.coverage.reasons.length > 10) {
      out.push(`  - ほか ${report.coverage.reasons.length - 10} 件（「診断と制限」を参照）`);
    }
  } else out.push('- 重要な未解決理由: なし');
  out.push('');
  ...
  report.paths.forEach((item: ModelPath, index) => {
    ...
    for (const reason of item.coverageReasons) out.push(`  - coverage 理由: ${escapeInline(reason)}`);
    out.push(`- 宣言元: ${item.declarationIds.length ? item.declarationIds.map(id => `${title(id)}（${links(nodes.get(id)?.evidenceIds ?? [])}）`).join(', ') : 'なし'}`);
    for (const id of item.edgeIds) if (pathOnlyEdges.has(id)) out.push(edgeLine(id));
    out.push('');
  });
  ...
  sectionIds.forEach((id, index) => {
    ...
    for (const edgeId of sectionEdges.get(id) ?? []) out.push(edgeLine(edgeId));
    out.push('');
  });
  if (!sectionIds.length) out.push('表示経路が確定しなかったため、番号付きの節は作らない。', '');
  ...
  for (const operation of report.operations) {
    ...
    for (const reason of operation.coverageReasons) out.push(`  - coverage 理由: ${escapeInline(reason)}`);
    for (const id of operation.edgeIds) if (!excluded.has(id)) out.push(edgeLine(id));
    out.push('');
  }
  ...
  if (httpEdges.length) {
    ...
    for (const edge of httpEdges) out.push(edgeLine(edge.id));
  } else {
  ...
  if (background.length) for (const edge of background) out.push(edgeLine(edge.id));
  else out.push('選択した経路・イベントの外から入る関係は検出していない。');
  out.push('');
  ...
  for (const item of report.coverage.events) out.push(`- イベント ${escapeInline(item.event)}: ${item.coverage}`);
  for (const reason of report.coverage.reasons) out.push(`- 理由: ${escapeInline(reason)}`);
  out.push('');
  ...
  const gapLine = (gap: CoverageGap): string => `- ${escapeInline(gap.code)}: ${escapeInline(gap.message)}` +
    `${gap.owner ? `（owner ${escapeInline(gap.owner)}）` : ''}`;
  out.push('### 未検出範囲', '');
  out.push('関連する未検出:');
  if (active.length) for (const gap of active) out.push(gapLine(gap));
  else out.push('- なし');
  ...
  if (globalUnknown.length) {
    for (const gap of globalUnknown) out.push(`- ${escapeInline(gap.code)}: ${escapeInline(gap.message)}`);
  } else out.push('- なし');
  out.push('', '対象外の未検出（コード別件数）:');
  if (report.coverage.gapCounts.length) {
    for (const item of report.coverage.gapCounts) out.push(`- ${escapeInline(item.code)}: ${item.count} 件`);
  } else out.push('- なし');
  out.push('');
  ...
  for (const edge of report.edges) {
    if (!rendered.has(edge.id)) problems.push(`Edge ${edge.id} (${edge.kind}) was not rendered`);
  }
