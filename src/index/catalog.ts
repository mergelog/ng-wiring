...
export interface Declaration {
  ...
  hostDirectiveExposures: Map<string, { inputs: Map<string, string>; outputs: Map<string, string> }>;
  inputs: Map<string, string>;
  outputs: Map<string, string>;
  template: { kind: 'inline' | 'external' | 'none'; text: string; file: string; expression?: ts.StringLiteralLike };
  gaps: string[];
}
...
function decorator(context: AnalysisContext, node: ts.ClassDeclaration): { kind: DeclarationKind; metadata: ts.ObjectLiteralExpression } | undefined {
  ...
  for (const item of t.getDecorators(node) ?? []) {
    if (!t.isCallExpression(item.expression) || item.expression.arguments.length !== 1) continue;
    const callee = item.expression.expression;
    const nameNode = t.isPropertyAccessExpression(callee) ? callee.name : callee;
    if (!t.isIdentifier(nameNode)) continue;
    ...
    if (!kind || !symbol?.declarations?.some(d => slash(d.getSourceFile().fileName).includes('/node_modules/@angular/core/'))) continue;
    const arg = unwrap(t, item.expression.arguments[0]!);
    if (t.isObjectLiteralExpression(arg)) return { kind, metadata: arg };
  }
  return undefined;
}
function refs(context: AnalysisContext, expression: ts.Expression | undefined, gaps: string[], label: string): string[] {
  if (!expression) return [];
  const t = context.toolchain.typescript;
  const visited = new Set<ts.Node>();
  const visit = (node: ts.Expression): string[] => {
    node = unwrap(t, node);
    if (visited.has(node)) { gaps.push(`${label}: circular metadata`); return []; }
    visited.add(node);
    ...
    if (declaration?.name) return [idForClass(context, declaration)!];
    if (t.isIdentifier(node)) {
      ...
      if (value && t.isVariableDeclaration(value) && value.initializer &&
        t.isVariableDeclarationList(value.parent) && value.parent.flags & t.NodeFlags.Const) return visit(value.initializer);
    }
...
function aliases(context: AnalysisContext, expression: ts.Expression | undefined): Map<string, string> {
  const result = new Map<string, string>();
  const t = context.toolchain.typescript;
  const value = expression && unwrap(t, expression);
  if (!value || !t.isArrayLiteralExpression(value)) return result;
  for (const item of value.elements) {
    if (!t.isStringLiteralLike(item)) continue;
    const [member, alias] = item.text.split(':').map(part => part.trim());
    if (member) result.set(alias || member, member);
  }
  return result;
}
function memberAliases(context: AnalysisContext, node: ts.ClassDeclaration,
  kind: 'Input' | 'Output', active = new Set<ts.ClassDeclaration>()): Map<string, string> {
  const result = new Map<string, string>();
  if (active.has(node)) return result;
  active.add(node);
  const t = context.toolchain.typescript;
  for (const heritage of node.heritageClauses ?? []) if (heritage.token === t.SyntaxKind.ExtendsKeyword) {
    for (const base of heritage.types) {
      ...
      if (inherited) for (const [alias, member] of memberAliases(context, inherited, kind, active)) result.set(alias, member);
    }
  }
  for (const member of node.members) {
    if (!member.name || !(t.isIdentifier(member.name) || t.isStringLiteral(member.name))) continue;
    const name = member.name.text;
    for (const item of t.canHaveDecorators(member) ? t.getDecorators(member) ?? [] : []) {
      if (!t.isCallExpression(item.expression)) continue;
      const callee = item.expression.expression;
    ...
    if (kind === 'Input' ? !['input', 'model'].includes(exportName ?? '') : !['output', 'model'].includes(exportName ?? '')) continue;
...
export async function buildCatalog(context: AnalysisContext): Promise<Catalog> {
  const t = context.toolchain.typescript;
  const evaluator = new StaticEvaluator(t, context.checker);
  const declarations = new Map<string, Declaration>();
  const external = new Map<string, ExternalDeclaration>();
  const byNode = new Map<ts.ClassDeclaration, Declaration>();
  const gaps: string[] = [];
    ...
    const visit = (node: ts.Node): void => {
      if (t.isClassDeclaration(node)) {
        ...
        if (found && node.name) {
          ...
          const prop = (name: string) => getProperty(t, found.metadata, name);
          if (prop('host')) localGaps.push('Host attributes are outside static template attribute matching');
          const scanRuntime = (part: ts.Node): void => {
            if (t.isCallExpression(part) && t.isPropertyAccessExpression(part.expression) &&
              part.expression.name.text === 'setAttribute') localGaps.push('Runtime setAttribute is outside static template attribute matching');
            t.forEachChild(part, scanRuntime);
          };
          scanRuntime(node);
          const evaluate = (name: string) => { const value = prop(name); return value ? evaluator.evaluate(value) : undefined; };
          ...
          if ((found.kind === 'component' || found.kind === 'directive') && !selector && prop('selector')) localGaps.push('Unresolved selector');
          ...
          let template: Declaration['template'] = { kind: 'none', text: '', file };
          if (found.kind === 'component' && inline) {
            const result = evaluator.evaluate(inline);
            if (result.known && typeof result.value === 'string') template = { kind: 'inline', text: result.value, file,
              expression: t.isStringLiteralLike(inline) ? inline : undefined };
            else localGaps.push('Unresolved inline template');
          ...
          } else if (found.kind === 'component' && prop('templateUrl')) localGaps.push('Unresolved templateUrl');
          const declaration: Declaration = {
            ...
            hostDirectiveExposures: (() => {
              const exposed = new Map<string, { inputs: Map<string, string>; outputs: Map<string, string> }>();
              const value = prop('hostDirectives');
              if (!value || !t.isArrayLiteralExpression(value)) return exposed;
              for (const item of value.elements) {
                if (!t.isObjectLiteralExpression(item)) continue;
                const directive = getProperty(t, item, 'directive');
                const id = directive && refs(context, directive, localGaps, 'hostDirectives')[0];
                if (id) exposed.set(id, { inputs: aliases(context, getProperty(t, item, 'inputs')),
  ...
  const referenceIds = [...declarations.values()].flatMap(item =>
    [...item.imports, ...item.declarations, ...item.exports, ...item.hostDirectives]);
  const queue = referenceIds.filter(id => id.startsWith('external:'));
  const typeRefs = (type: ts.TypeNode | undefined): string[] => {
    if (!type) return [];
    if (t.isTupleTypeNode(type)) return type.elements.flatMap(typeRefs);
    if (t.isUnionTypeNode(type)) return type.types.flatMap(typeRefs);
  ...
  while (queue.length && external.size < 2_000) {
    ...
    if (!node || !t.isClassDeclaration(node)) { gaps.push(`External metadata unavailable: ${id}`); continue; }
    ...
    if (!metadata || !t.isPropertyDeclaration(metadata) || !metadata.type || !t.isTypeReferenceNode(metadata.type)) {
      gaps.push(`External Angular metadata unavailable: ${id}`); continue;
    }
    const field = metadata.name.getText();
    const kind: DeclarationKind = field === 'ɵcmp' ? 'component' : field === 'ɵdir' ? 'directive' :
    ...
    const aliasMap = (type: ts.TypeNode | undefined): Map<string, string> => {
      const result = new Map<string, string>();
      if (!type || !t.isTypeLiteralNode(type)) return result;
      for (const member of type.members) {
        if (!t.isPropertySignature(member) || !member.name) continue;
        const name = member.name.getText().replace(/^['"]|['"]$/g, '');
        let alias: string | undefined;
        if (member.type && t.isLiteralTypeNode(member.type) && t.isStringLiteral(member.type.literal)) alias = member.type.literal.text;
        if (member.type && t.isTypeLiteralNode(member.type)) {
          ...
          if (prop && t.isPropertySignature(prop) && prop.type && t.isLiteralTypeNode(prop.type) &&
            t.isStringLiteral(prop.type.literal)) alias = prop.type.literal.text;
        }
  ...
  if (queue.length) gaps.push('External metadata expansion limit');
