...
import { matchIdentifier, type Capability } from './capabilities.js';
...
export interface StoreHook { kind: 'onInit' | 'onDestroy'; source: string; featureIndex: number }
...
function matchedCapability(context: AnalysisContext, call: ts.CallExpression): Capability | null {
...
function producedObject(context: AnalysisContext, expression: ts.Expression | undefined): ts.ObjectLiteralExpression | null {
  if (!expression) return null;
  const t = context.toolchain.typescript;
  const node = definition(context, expression);
  if (t.isObjectLiteralExpression(node)) return node;
  if (!t.isArrowFunction(node) && !t.isFunctionExpression(node)) return null;
...
function memberNames(context: AnalysisContext, object: ts.ObjectLiteralExpression | null,
  gaps: string[], label: string): string[] {
  const t = context.toolchain.typescript;
  if (!object) { gaps.push(`${label} members are not a statically readable object`); return []; }
  const names: string[] = [];
...
function featureCall(context: AnalysisContext, expression: ts.Expression, depth: number): ts.CallExpression | null {
  const t = context.toolchain.typescript;
  const node = definition(context, expression);
  if (!t.isCallExpression(node)) return null;
  if (matchedCapability(context, node) || depth >= 16) return node;
...
function collectFeatures(context: AnalysisContext, args: readonly ts.Expression[], collector: Collector,
  ...
  for (const argument of args) {
    ...
    const push = (status: StoreFeature['status'] = 'resolved', reason: string | null = null): void => {
      collector.features.push({ index, capability: capability.matcherId, label: capability.export ?? '', source,
        status, reason });
    };
    switch (capability.matcherId) {
      case 'signals/signalStoreFeature': {
        push();
        if (depth >= 16) { collector.gaps.push(`feature composition depth limit at ${source}`); break; }
      ...
      case 'signals/withProps':
        ...
        addMembers(collector, memberNames(context, producedObject(context, call.arguments[0]), collector.gaps,
          'withProps'), 'prop', index, capability.matcherId, source);
        break;
      case 'signals/withMethods':
        push();
        addMembers(collector, memberNames(context, producedObject(context, call.arguments[0]), collector.gaps,
          'withMethods'), 'method', index, capability.matcherId, source);
        break;
      case 'signals/withHooks': {
        push();
        const object = producedObject(context, call.arguments[0]);
        for (const name of ['onInit', 'onDestroy'] as const) {
          const property = object?.properties.find(item => item.name && t.isIdentifier(item.name) &&
            item.name.text === name);
          if (property) collector.hooks.push({ kind: name, source: location(context, property), featureIndex: index });
        }
        if (!object) collector.gaps.push(`withHooks at ${source} has no statically readable hook object`);
        break;
      }
...
function producedFeature(context: AnalysisContext, expression: ts.Expression): ts.Expression | null {
  const t = context.toolchain.typescript;
  const node = definition(context, expression);
  if (!t.isArrowFunction(node) && !t.isFunctionExpression(node)) return node;
  if (!t.isBlock(node.body)) return unwrap(t, node.body);
  return node.body.statements.find(t.isReturnStatement)?.expression ?? null;
}

function storeId(context: AnalysisContext, node: ts.Node, name: string): string {
  return `${slash(node.getSourceFile().fileName)}:${node.getStart()}:${name}@${context.id}`;
}
function build(context: AnalysisContext, call: ts.CallExpression, anchor: ts.Node, name: string,
  kind: SignalStoreDeclaration['kind']): SignalStoreDeclaration {
  const collector: Collector = { features: [], members: [], stateKeys: [], hooks: [], gaps: [] };
...
export function catalogSignalStores(context: AnalysisContext): SignalStoreCatalog {
  const t = context.toolchain.typescript;
  const declarations = new Map<string, SignalStoreDeclaration>();
  const byNode = new Map<ts.Node, string>();
  const diagnostics: string[] = [];
  const isSignalStore = (expression: ts.Expression): ts.CallExpression | null => {
    const node = definition(context, expression);
    if (!t.isCallExpression(node)) return null;
  ...
  const declarationFor = (expression: ts.Expression): string | null => {
  ...
  const instances: SignalStoreInstance[] = [];
  ...
  for (const fileName of context.sourceFiles) {
    ...
    const visit = (node: ts.Node): void => {
      if (t.isCallExpression(node)) {
        ...
        if (symbol?.getName() === 'inject' && symbol.declarations?.some(part =>
          slash(part.getSourceFile().fileName).includes('/node_modules/@angular/core/'))) {
          const token = node.arguments[0];
          const declarationId = token && declarationFor(token);
          if (declarationId) instances.push({ id: location(context, node), declarationId, kind: 'inject',
      ...
      if (t.isNewExpression(node)) {
        ...
        if (declarationId) instances.push({ id: location(context, node), declarationId, kind: 'new',
      ...
      if (t.isPropertyAssignment(node) && t.isIdentifier(node.name) &&
        ...
        if (t.isArrayLiteralExpression(list)) for (const element of list.elements) {
  ...
  for (const record of declarations.values())
    if (record.status === 'partial') diagnostics.push(`Store ${record.name} has ${record.gaps.length} unresolved feature range(s)`);
...
export function resolveStoreReference(context: AnalysisContext, catalog: SignalStoreCatalog,
  node: ts.Node): StoreReference | null {
  const t = context.toolchain.typescript;
  const declaration = symbolOf(context, node)?.valueDeclaration;
  if (!declaration) return null;
  const fromExpression = (expression: ts.Expression, member: string | null): StoreReference | null => {
    const instance = catalog.instances.find(item => item.source === location(context, expression));
    if (instance) return { kind: 'instance', declarationId: instance.declarationId, instanceId: instance.id,
    ...
    if (t.isIdentifier(expression) || t.isPropertyAccessExpression(expression)) {
      ...
      if (next && next !== declaration) return resolveMember(next, member);
    }
    return null;
  };
  const resolveMember = (target: ts.Declaration, member: string | null): StoreReference | null => {
    ...
    if (t.isBindingElement(target)) {
      ...
      if (t.isVariableDeclaration(parent) && parent.initializer) return fromExpression(parent.initializer, field ?? member);
      if (t.isParameter(parent)) return fromParameter(parent, field ?? member);
    }
    if (t.isParameter(target)) return fromParameter(target, member);
    return null;
  };
  /** The first parameter of a feature factory is the Store under construction, not a separate value. */
  const fromParameter = (parameter: ts.ParameterDeclaration, member: string | null): StoreReference | null => {
    const owner = parameter.parent;
    if (!t.isArrowFunction(owner) && !t.isFunctionExpression(owner)) return null;
    if (owner.parameters[0] !== parameter) return null;
    let cursor: ts.Node | undefined = owner.parent;
    while (cursor && !t.isCallExpression(cursor)) cursor = cursor.parent;
    if (!cursor || !t.isCallExpression(cursor)) return null;
...
export interface StoreLifetime { declarationId: string; instanceId: string | null; created: boolean;
  start: string[]; end: string[]; conditions: string[] }
/** onInit is a start condition and onDestroy an end condition; neither is a result of a UI operation. */
export function storeLifetime(catalog: SignalStoreCatalog, declarationId: string,
  instanceId: string | null = null): StoreLifetime {
