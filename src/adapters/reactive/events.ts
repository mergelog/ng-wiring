...
export interface EventConsumer {
  ...
  redelivers: { creatorId: string | null; creatorType: string | null; scope: DeliveryScope; source: string }[];
  gaps: string[];
}
...
function calleeOf(context: AnalysisContext, call: ts.CallExpression): ts.Node {
  const t = context.toolchain.typescript;
  return t.isPropertyAccessExpression(call.expression) ? call.expression.name : call.expression;
}
function matcherOf(context: AnalysisContext, call: ts.CallExpression): string | null {
  return matchIdentifier(context, calleeOf(context, call))?.capability.matcherId ?? null;
}
function declarationId(context: AnalysisContext, node: ts.Node): string {
...
function stringOf(context: AnalysisContext, expression: ts.Expression | undefined): string | null {
...
function scopeOf(context: AnalysisContext, config: ts.Expression | undefined): DeliveryScope {
  const t = context.toolchain.typescript;
...
function walk(context: AnalysisContext, node: ts.Node, visit: (child: ts.Node) => void): void {
  const t = context.toolchain.typescript;
  const step = (child: ts.Node): void => { visit(child); t.forEachChild(child, step); };
  step(node);
}

/** `injectDispatch(group)` and `injectDispatch(group)({scope})` both name events from the same group. */
function injectDispatchTarget(context: AnalysisContext, receiver: ts.Expression):
  { group: ts.Expression; scope: DeliveryScope } | null {
  const t = context.toolchain.typescript;
...
function groupMembers(context: AnalysisContext, group: ts.Expression): Map<string, EventCreator> {
  const t = context.toolchain.typescript;
  const result = new Map<string, EventCreator>();
  const node = definition(context, group);
  if (!t.isCallExpression(node) || matcherOf(context, node) !== 'signals-events/eventGroup') return result;
  const config = node.arguments[0] && definition(context, node.arguments[0]);
  if (!config || !t.isObjectLiteralExpression(config)) return result;
  ...
  if (!list || !t.isObjectLiteralExpression(list)) return result;
  const groupId = declarationId(context, group);
  for (const property of list.properties) {
    const name = property.name;
    if (!name || (!t.isIdentifier(name) && !t.isStringLiteralLike(name))) continue;
...
type Redelivery = { creatorId: string | null; creatorType: string | null; scope: DeliveryScope; source: string };
...
function emittedEvents(context: AnalysisContext, pipeline: ts.Node, creators: Map<string, EventCreator>,
  received: (EventCreator | null)[] = [], subscriptionSource: ts.Node | null = null): Redelivery[] {
  const t = context.toolchain.typescript;
  const projections = new Set(['map', 'mergeMap', 'switchMap', 'concatMap', 'exhaustMap', 'tapResponse', 'mapResponse']);
  const result: Redelivery[] = [];
  let mapped: DeliveryScope | null = null;
  const passThrough = (at: ts.Node, scope: DeliveryScope): void => {
  ...
  const fromValue = (value: ts.Expression, scope: DeliveryScope): void => {
    ...
    if (t.isArrayLiteralExpression(node)) {
      ...
      for (const element of node.elements) {
        const inner = definition(context, element);
        if (t.isCallExpression(inner) && matcherOf(context, inner) === 'signals-events/toScope') continue;
        fromValue(element, nextScope);
      }
      return;
    }
    if (t.isConditionalExpression(node)) { fromValue(node.whenTrue, scope); fromValue(node.whenFalse, scope); return; }
    if (!t.isCallExpression(node)) return;
    const id = declarationId(context, node.expression);
    const creator = creators.get(id);
    if (creator) { result.push({ creatorId: creator.id, creatorType: creator.type, scope,
      source: location(context, node) }); return; }
    ...
    if (group) result.push({ creatorId: group.id, creatorType: group.type, scope, source: location(context, node) });
  };
  ...
  walk(context, pipeline, child => {
    ...
    if (!projections.has(name)) return;
    for (const argument of child.arguments) {
      ...
      if (t.isObjectLiteralExpression(callback)) {
        for (const property of callback.properties)
          if (t.isPropertyAssignment(property)) fromValue(property.initializer, mapped ?? 'self');
        continue;
      }
      if (!t.isArrowFunction(callback) && !t.isFunctionExpression(callback)) continue;
      const parameter = callback.parameters[0]?.name;
      const isReceived = (value: ts.Expression): boolean => t.isIdentifier(value) && !!parameter &&
      ...
      for (const value of produced) {
        if (isReceived(unwrap(t, value))) passThrough(value, mapped ?? 'self');
        else fromValue(value, mapped ?? 'self');
      }
    }
  });
  // An `Events.on(...)` with no operators emits the received events, and those are dispatched again.
  if (pipeline === subscriptionSource && received.length) passThrough(pipeline, mapped ?? 'self');
  return result;
}

/** Catalogues the SignalStore event bus: creators, send sites, consumers, and explicit bridges. */
export function analyzeEvents(context: AnalysisContext, stores: SignalStoreCatalog): EventGraph {
  const t = context.toolchain.typescript;
  const creators = new Map<string, EventCreator>();
  const dispatches: EventDispatch[] = [];
  const consumers: EventConsumer[] = [];
  const bridges: EventBridge[] = [];
  const crossBus: CrossBusSend[] = [];
  const dispatcherOwners = new Set<string>();
  const diagnostics: string[] = [];
  const files = context.sourceFiles.map(name => context.program.getSourceFile(name))
    .filter((file): file is ts.SourceFile => !!file);
  for (const file of files) walk(context, file, node => {
    if (!t.isVariableDeclaration(node) || !node.initializer || !t.isIdentifier(node.name)) return;
    ...
    if (matcherOf(context, call) === 'signals-events/eventGroup')
      for (const member of groupMembers(context, node.name).values()) creators.set(member.id, member);
  });
  const creatorFor = (expression: ts.Expression): EventCreator | null => {
    const node = definition(context, expression);
    if (!t.isCallExpression(node)) return null;
    const direct = creators.get(declarationId(context, node.expression));
    if (direct) return direct;
    if (!t.isPropertyAccessExpression(node.expression)) return null;
    return groupMembers(context, node.expression.expression).get(node.expression.name.text) ?? null;
  };
  const storeFeatureSources = new Map<string, string>();
  for (const record of stores.declarations.values())
    for (const feature of record.features) storeFeatureSources.set(feature.source, record.id);
  ...
  for (const file of files) walk(context, file, node => {
    if (t.isCallExpression(node) && matcherOf(context, node) === 'signals-events/provideDispatcher') {
      ...
      if (owner) dispatcherOwners.add(owner);
      else diagnostics.push(`provideDispatcher at ${location(context, node)} has no resolved owning class`);
    }
    if (!t.isCallExpression(node) || !t.isPropertyAccessExpression(node.expression)) return;
  ...
  for (const file of files) walk(context, file, node => {
        ...
        for (const value of produced) {
          if (t.isObjectLiteralExpression(value)) {
            for (const property of value.properties) {
              const name = property.name;
              if (name && (t.isIdentifier(name) || t.isStringLiteralLike(name))) writes.push(name.text);
              else gaps.push('reducer result has a member name that is not statically readable');
            }
          } else gaps.push('reducer result is a partial state updater whose written keys are not statically readable');
        }
    ...
    if (!t.isPropertyAccessExpression(node.expression)) return;
    ...
    if (on?.capability.matcherId !== 'signals-events/Events.on' &&
      on?.capability.matcherId !== 'signals-events/ReducerEvents.on') return;
    if (enclosingStore(node)) return;
    collectOne(node, 'subscription', null, ['the subscription must be registered and still alive']);
  });
  function collectSubscriptions(feature: ts.CallExpression, kind: EventConsumerKind, storeId: string | null,
    conditions: string[]): void {
  ...
  function collectOne(call: ts.CallExpression, kind: EventConsumerKind, storeId: string | null,
    conditions: string[]): void {
    const handled = call.arguments.map(item => creatorFor(item) ?? creators.get(declarationId(context, item)) ??
      (t.isPropertyAccessExpression(item) ? groupMembers(context, item.expression).get(item.name.text) : null) ?? null);
    let pipeline: ts.Node = call;
    ...
    const consumer: EventConsumer = { id: location(context, call), kind,
      creatorIds: handled.flatMap(item => item ? [item.id] : []),
      creatorTypes: handled.flatMap(item => item?.type ? [item.type] : []),
    ...
    walk(context, pipeline, child => {
      ...
      if (bridged?.capability.matcherId !== 'ngrx-store/Store.dispatch' &&
        bridged?.capability.matcherId !== 'ngrx-store/Store.next') return;
  ...
  for (const file of files) walk(context, file, node => {
    ...
    if (member?.capability.matcherId !== 'ngrx-store/Store.dispatch' &&
      member?.capability.matcherId !== 'ngrx-store/Store.next') return;
    const argument = node.arguments[0];
    const creator = argument && creatorFor(argument);
    if (!creator) return;
    const place = location(context, node);
    if (bridges.some(item => item.source === place)) return;
  ...
  for (const dispatch of dispatches)
    if (dispatch.status === 'boundary' && dispatch.reason) diagnostics.push(`${dispatch.reason} at ${dispatch.source}`);
  for (const send of crossBus) diagnostics.push(`${send.creatorType ?? send.creatorId}: ${send.reason} at ${send.source}`);
...
export function resolveEventDelivery(graph: EventGraph, dispatch: EventDispatch,
  ...
  for (const consumer of graph.consumers) {
    if (consumer.bus !== dispatch.bus) continue;
    ...
    if (consumerBus === null) { reasons.push(`consumer ${consumer.id} has no resolved bus instance`); continue; }
    if (consumerBus !== busId) continue;
...
export function busOfCapability(matcherId: string): DeliveryBus | null {
  if (matcherId.startsWith('signals-events/')) return 'signal-store-event';
  if (matcherId.startsWith('ngrx-store/') || matcherId.startsWith('ngrx-effects/')) return 'ngrx-action';
  return null;
}
