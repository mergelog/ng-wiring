...
import { matchIdentifier, type ReactiveFramework } from './capabilities.js';
...
function isSignalValue(context: AnalysisContext, node: ts.Expression): boolean {
  const type = context.checker.getTypeAtLocation(node);
  const check = (candidate: ts.Type): boolean => {
    const symbol = candidate.getSymbol() ?? candidate.aliasSymbol;
    if (!symbol) return false;
    if (!signalTypeNames.has(symbol.getName())) return false;
...
function nameOf(context: AnalysisContext, node: ts.Node): string | null {
  const t = context.toolchain.typescript;
  const parent = node.parent;
  if (t.isVariableDeclaration(parent) && t.isIdentifier(parent.name)) return parent.name.text;
...
function objectKeys(context: AnalysisContext, expression: ts.Expression | undefined, into: string[]): void {
  ...
  for (const property of node.properties) {
    ...
    if (name && (t.isIdentifier(name) || t.isStringLiteralLike(name))) into.push(name.text);
  }
}
...
function trackingAt(context: AnalysisContext, node: ts.Node): { tracking: ReadTracking; reason: string | null } {
  const t = context.toolchain.typescript;
  let cursor: ts.Node | undefined = node;
  ...
  while (cursor) {
    if (t.isCallExpression(cursor) && cursor.arguments.some(argument => argument === child)) {
      ...
      if (matcher === 'angular/untracked') return { tracking: 'untracked', reason: null };
      ...
      if (matcher === 'signals/getState') return { tracking: 'snapshot', reason: null };
    }
    child = cursor;
    cursor = cursor.parent;
  }
...
function hasAwaitBefore(context: AnalysisContext, callback: ts.Node, position: number): boolean {
  const t = context.toolchain.typescript;
  let found = false;
  const visit = (node: ts.Node): void => {
    if (found) return;
    if (t.isAwaitExpression(node) && node.getEnd() <= position) { found = true; return; }
...
function equalityArgument(context: AnalysisContext, matcher: string, call: ts.CallExpression): string | null {
  const t = context.toolchain.typescript;
  const index = matcher === 'angular/linkedSignal' && call.arguments.length === 1 ? 0 : 1;
  const candidates = [call.arguments[index], call.arguments[0]].filter((item): item is ts.Expression => !!item);
  for (const candidate of candidates) {
    const options = unwrap(t, candidate);
    if (!t.isObjectLiteralExpression(options)) continue;
...
export function analyzeSignals(context: AnalysisContext, files?: readonly ts.SourceFile[]): SignalGraph {
  const t = context.toolchain.typescript;
  ...
  for (const file of targets) walk(file, node => {
    ...
    if (framework) {
      ...
      const keys: string[] = [];
      if (matcher === 'signals/signalState') objectKeys(context, node.arguments[0], keys);
      const record: SignalSource = { id: location(context, node), capability: matcher,
    ...
    if (matcher === 'angular/computed' || matcher === 'angular/linkedSignal' || matcher === 'signals/deepComputed' ||
      ...
      const record: SignalLink = { id: location(context, node), capability: matcher, from: adapterSource.id,
  ...
  for (const file of targets) walk(file, node => {
    ...
    if (matcher === 'signals/patchState') {
      ...
      for (const updater of node.arguments.slice(1)) {
        ...
        if (t.isObjectLiteralExpression(value)) objectKeys(context, value, keys);
          ...
          for (const item of produced) {
            ...
            if (t.isObjectLiteralExpression(inner)) objectKeys(context, inner, keys);
            else gaps.push('a state updater result is not a statically readable object');
          }
        } else gaps.push('a patchState argument is not statically readable');
      }
  ...
  for (const effect of effects) {
    ...
    if (parameter && t.isIdentifier(parameter)) walk(node, child => {
