...
export function eventDeliverySteps(graph: EventGraph, dispatch: EventDispatch,
  resolution: DeliveryResolution): ReactiveStep[] {
  const delivery = (registration: string[]): DeliveryDetails => ({ bus: 'signal-store-event',
    busId: resolution.busId, form: dispatch.form, scope: dispatch.scope, registration });
  const steps: ReactiveStep[] = [{ kind: 'event-dispatch', source: dispatch.owner ?? dispatch.source,
...
export function actionDeliverySteps(graph: StoreGraph, action: StoreAction, source: string,
  form: DeliveryDetails['form'] = 'action-instance', conditions: string[] = []): ReactiveStep[] {
  ...
  const delivery = (extra: string[]): DeliveryDetails => ({ bus: 'ngrx-action', busId: 'root', form,
    scope: 'root', registration: [...registration, ...extra] });
  const steps: ReactiveStep[] = [{ kind: 'action-dispatch', source, target: action.type ?? action.id,
