...
const ledger: ReactiveCase[] = [
  ...
  entry({ contract: 'R05', id: 'R05/signalStore.class-extends', package: '@ngrx/signals', export: 'signalStore',
    ...
    demonstratedBy: storeApis('data-id=shoutButton'),
    missingFixture: 'signal-store-apis の ExtendedCatalogStore はカタログでは解決されるが、method 呼出しが extends 越しに解決されず境界になる（R05 未達）' }),
  ...
  entry({ contract: 'R05', id: 'R05/withFeature', package: '@ngrx/signals', export: 'withFeature', version: NGRX,
    ...
    demonstratedBy: storeApis('data-id=shoutButton'),
    missingFixture: 'signal-store-apis の withFeature が提供する shouted に到達できない（method 解決の境界が手前にある）' }),
  ...
  entry({ contract: 'R05', id: 'R05/signalStore.provider-instance', package: '@ngrx/signals', export: 'signalStore',
    ...
    missingFixture: 'state ノードは宣言と key で識別されるため、provider ごとのインスタンスを分離できない（R05 未達）' }),
