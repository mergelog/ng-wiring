...
const eventsMapToScope = (target: string): FixtureRef => ({ id: 'events-map-to-scope', target });
...
const ledger: ReactiveCase[] = [
  ...
  entry({ contract: 'R04', id: 'R04/toSignal', package: '@angular/core/rxjs-interop', export: 'toSignal',
    ...
    fixture: interopApis('data-id=commitButton'),
    expectedEdges: ['reactive-link|clock.stream|tick', 'state-read|tick|<span>'],
    expectedConditions: ['the internal subscription starts when the toSignal call runs',
      'the internal subscription ends when the owning injection context is destroyed'],
    expectedDetails: [{ edge: 'reactive-link|clock.stream|tick', key: 'operator', value: 'angular/toSignal' },
      { edge: 'reactive-link|clock.stream|tick', key: 'scheduling', value: 'subscription' }],
    forbiddenEdges: [{ kind: 'reactive-link', from: 'value', to: 'tick',
      reason: '選択した commit の write を独立した Observable の発火と誤認しない' }] }),
  entry({ contract: 'R04', id: 'R04/toObservable', package: '@angular/core/rxjs-interop', export: 'toObservable',
    version: ANGULAR, expectation: 'Signal の write から Observable への変換を接続し、set と通知が同数になると仮定しない',
    fixture: interopApis('data-id=commitButton'),
    expectedEdges: ['state-write|click → commit()|value', 'reactive-link|value|value$'],
    expectedConditions: ['each notification follows a change-detection boundary, not every set'],
    expectedDetails: [{ edge: 'reactive-link|value|value$', key: 'operator', value: 'angular/toObservable' },
      { edge: 'reactive-link|value|value$', key: 'scheduling', value: 'change-detection boundary' }] }),
  ...
  entry({ contract: 'R05', id: 'R05/signalStore.class-extends', package: '@ngrx/signals', export: 'signalStore',
    ...
    fixture: storeApis('data-id=shoutButton'),
    expectedEdges: ['call|src/extended.component.ts#ExtendedComponent|store.shout',
      'state-write|click → shout()|term', 'state-read|term|<span>'] }),
  ...
  entry({ contract: 'R05', id: 'R05/signalStore.provider-instance', package: '@ngrx/signals', export: 'signalStore',
    ...
    fixture: storeApis('data-id=peerSetTermButton'),
    expectedEdges: ['call|src/catalog-peer.component.ts#CatalogPeerComponent|store.setTerm',
      'state-write|click → setTerm()|term', 'state-read|term|<span>'],
    expectedNodes: [{ kind: 'state', label: 'term' }] }),
  ...
  entry({ contract: 'R06', id: 'R06/withComputed', package: '@ngrx/signals', export: 'withComputed', version: NGRX,
    ...
    fixture: storeApis('data-id=setTermButton'),
    expectedEdges: ['state-write|click → setTerm()|term', 'reactive-link|term|label', 'state-read|label|<span>'],
    expectedConditions: ['the value is recomputed lazily when read', 'a tracked dependency must change its compared value'],
    expectedDetails: [{ edge: 'reactive-link|term|label', key: 'operator', value: 'signals/withComputed' }] }),
  entry({ contract: 'R06', id: 'R06/withLinkedState', package: '@ngrx/signals', export: 'withLinkedState',
    ...
    fixture: storeApis('data-id=setTermButton'),
    expectedEdges: ['reactive-link|term|draft', 'state-write|click → setTerm()|draft', 'state-read|draft|<span>'],
    expectedConditions: ['the value is recomputed when its source changes', 'an explicit write also replaces the value'],
    expectedDetails: [{ edge: 'reactive-link|term|draft', key: 'scheduling', value: 'source change or explicit write' }] }),
  ...
  entry({ contract: 'R06', id: 'R06/withHooks', package: '@ngrx/signals', export: 'withHooks', version: NGRX,
    ...
    fixture: storeApis('data-id=setTermButton'),
    expectedEdges: ['state-write|click → setTerm()|term'],
    expectedConditions: ['Store starts at onInit at', 'Store ends at onDestroy at'] }),
  entry({ contract: 'R06', kind: 'counter-example', id: 'R06/withHooks.not-created', package: '@ngrx/signals', export: 'withHooks',
    ...
    fixture: storeApis('data-id=shoutButton'),
    forbiddenEdges: [{ kind: 'state-write', to: 'idle', reason: '未生成 Store の state を書き換え対象にしない' },
      { kind: 'call', to: 'store.touch', reason: '未生成 Store の method を稼働中にしない' }] }),
  ...
  entry({ contract: 'R07', id: 'R07/watchState', package: '@ngrx/signals', export: 'watchState', version: NGRX,
    expectation: 'SignalState の write と監視を結び、初回通知と更新通知を区別する',
    fixture: storeApis('data-id=pageButton'),
    expectedEdges: ['state-write|click → nextPage()|filters', 'reactive-link|filters|signals/watchState'],
    expectedConditions: ['watchState emits an initial snapshot after registration',
      'subsequent notifications follow SignalState changes'],
    expectedDetails: [{ edge: 'reactive-link|filters|signals/watchState', key: 'scheduling', value: 'state-watcher' }] }),
  entry({ contract: 'R07', id: 'R07/deepComputed', package: '@ngrx/signals', export: 'deepComputed', version: NGRX,
    expectation: 'SignalState の深いプロパティの派生を reactive-link として残す',
    fixture: storeApis('data-id=pageButton'),
    expectedEdges: ['state-write|click → nextPage()|filters', 'reactive-link|filters|deep'],
    expectedConditions: ['the value is recomputed lazily when read',
      'a tracked dependency must change its compared value'],
    expectedDetails: [{ edge: 'reactive-link|filters|deep', key: 'operator', value: 'signals/deepComputed' }] }),
  ...
  entry({ contract: 'R08', id: 'R08/rxMethod.value', package: '@ngrx/signals/rxjs-interop', export: 'rxMethod',
    ...
    expectedEdges: ['call|src/methods.component.ts#MethodsComponent|store.load',
      'state-write|click → loadValue()|hits'],
    expectedConditions: ['the pipeline runs once for the supplied value'] }),
  entry({ contract: 'R08', id: 'R08/rxMethod.signal', package: '@ngrx/signals/rxjs-interop', export: 'rxMethod',
    ...
    fixture: storeApis('data-id=loadSignalButton'),
    expectedEdges: ['call|src/methods.component.ts#MethodsComponent|store.load',
      'state-write|click → loadSignal()|hits'],
    expectedConditions: ['the pipeline restarts on each change of the supplied Signal'] }),
  entry({ contract: 'R08', id: 'R08/rxMethod.observable', package: '@ngrx/signals/rxjs-interop', export: 'rxMethod',
    ...
    fixture: storeApis('data-id=loadStreamButton'),
    expectedEdges: ['call|src/methods.component.ts#MethodsComponent|store.load',
      'state-write|click → loadStream()|hits'],
    expectedConditions: ['the pipeline runs once per notification of the supplied Observable'] }),
  entry({ contract: 'R08', kind: 'counter-example', id: 'R08/rxMethod.uncalled', package: '@ngrx/signals/rxjs-interop', export: 'rxMethod',
    ...
    fixture: storeApis('data-id=loadValueButton'),
    forbiddenEdges: [{ kind: 'state-write', to: 'unusedHits', reason: '未呼出しの rxMethod を稼働中にしない' }] }),
  entry({ contract: 'R08', id: 'R08/signalMethod.value', package: '@ngrx/signals', export: 'signalMethod',
    ...
    fixture: storeApis('data-id=rememberButton'),
    expectedEdges: ['call|src/methods.component.ts#MethodsComponent|store.remember',
      'state-write|click → remember()|last'],
    expectedConditions: ['the processing function runs once for the supplied value'] }),
  entry({ contract: 'R08', id: 'R08/signalMethod.signal', package: '@ngrx/signals', export: 'signalMethod',
    ...
    expectedEdges: ['call|src/methods.component.ts#MethodsComponent|store.remember',
      'state-write|click → remember()|last'],
    expectedConditions: ['the processing function runs once per change of the supplied Signal'] }),
  entry({ contract: 'R08', kind: 'counter-example', id: 'R08/signalMethod.no-observable', package: '@ngrx/signals', export: 'signalMethod',
    ...
    expectedEdges: ['call|src/methods.component.ts#MethodsComponent|store.remember',
      'state-write|click → remember()|last'],
    expectedConditions: ['an Observable argument is not supported and is not inferred from rxMethod'],
    expectedDiagnostics: ['reactive-method-argument'],
    }),
  ...
  entry({ contract: 'R09', id: 'R09/Store.dispatch.action-object', package: '@ngrx/store', export: 'Store',
    ...
    fixture: ngrxApis('data-id=objectButton'),
    expectedEdges: ['action-dispatch|src/panel.component.ts#SearchPanelComponent|src/actions.ts#termChanged',
      'action-consume|src/actions.ts#termChanged|src/reducer.ts#searchReducer',
  ...
  entry({ contract: 'R09', id: 'R09/createActionGroup', package: '@ngrx/store', export: 'createActionGroup',
    ...
    fixture: ngrxApis('data-id=groupButton'),
    expectedEdges: ['action-dispatch|src/panel.component.ts#SearchPanelComponent|src/actions.ts:526:auditEvents:panelClosed',
      'action-consume|src/actions.ts:526:auditEvents:panelClosed|src/reducer.ts#auditReducer',
      'state-write|src/reducer.ts#auditReducer|audit'] }),
  ...
  entry({ contract: 'R09', id: 'R09/createFeature', package: '@ngrx/store', export: 'createFeature', version: NGRX,
    ...
    fixture: ngrxApis('data-id=featureButton'),
    expectedEdges: ['action-dispatch|src/panel.component.ts#SearchPanelComponent|src/actions.ts#noteAdded',
      'action-consume|src/actions.ts#noteAdded|src/reducer.ts#notesFeature',
      'state-write|src/reducer.ts#notesFeature|notes',
      'state-read|notes|src/reducer.ts:922:notesFeature.selectNotes'] }),
  entry({ contract: 'R09', id: 'R09/Store.select', package: '@ngrx/store', export: 'Store', member: 'select',
    ...
    expectedEdges: ['reactive-link|src/reducer.ts#selectTerm|src/panel.component.ts:30:19'] }),
  ...
  entry({ contract: 'R10', id: 'R10/Store.dispatch.thunk', package: '@ngrx/store', export: 'Store',
    ...
    fixture: ngrxApis('data-id=thunkButton'),
    expectedEdges: ['action-dispatch|src/panel.component.ts#SearchPanelComponent|src/actions.ts#termChanged',
      'action-consume|src/actions.ts#termChanged|src/reducer.ts#searchReducer',
      'state-write|src/reducer.ts#searchReducer|search'],
    expectedConditions: ['the dispatch function runs initially and again when a Signal read by it changes'],
    expectedDetails: [{ edge: 'action-dispatch|src/panel.component.ts#SearchPanelComponent|src/actions.ts#termChanged',
      key: 'dispatchMode', value: 'reactive-factory' }] }),
  entry({ contract: 'R10', id: 'R10/Store.dispatch.thunk-injector', package: '@ngrx/store', export: 'Store',
    ...
    fixture: ngrxApis('data-id=thunkButton'),
    expectedEdges: ['action-dispatch|src/panel.component.ts#SearchPanelComponent|src/actions.ts#termChanged'],
    expectedConditions: ['the dispatch registration uses this.injector; destroying it stops redispatch'] }),
  entry({ contract: 'R10', id: 'R10/Store.next', package: '@ngrx/store', export: 'Store', member: 'next',
    ...
    fixture: ngrxApis('data-id=nextButton'),
    expectedEdges: ['action-dispatch|src/panel.component.ts#SearchPanelComponent|src/actions.ts#termChanged',
      'action-consume|src/actions.ts#termChanged|src/reducer.ts#searchReducer',
      'state-write|src/reducer.ts#searchReducer|search'] }),
  ...
  entry({ contract: 'R14', id: 'R14/mapToScope', package: '@ngrx/signals/events', export: 'mapToScope', version: NGRX,
    expectation: '演算子による scope 変更と handler の再配送先を検知する',
    fixture: eventsMapToScope('data-id=pageButton'),
    expectedEdges: ['event-dispatch|src/grid.store.ts:20:19|[Grid] pageChanged'],
    expectedConditions: ['handlers are subscribed when the Store is created',
      'the Store instance must still be alive'],
    expectedDetails: [{ edge: 'event-dispatch|src/grid.store.ts:20:19|[Grid] pageChanged', key: 'scope', value: 'parent' },
      { edge: 'event-dispatch|src/grid.store.ts:20:19|[Grid] pageChanged', key: 'busId', value: 'root' }] }),
  ...
  entry({ contract: 'R15', id: 'R15/settings-store', package: '@ngrx/signals', export: 'withMethods', form: 'form',
    ...
    expectedEdges: ['call|src/settings.component.ts#ProjectSettingsComponent|store.loadScalars',
      'reactive-link|load|forkJoin', 'reactive-link|load|lastValueFrom',
      'http-create|src/metrics-api.ts#MetricsApiService|POST /api/settings/metrics',
      'http-consume|POST /api/settings/metrics|promise-consume',
      'state-write|click → load()|scalars', 'state-read|scalars|<span>'],
    expectedConditions: ['all input observables must complete',
      'subscribes and resolves/rejects a Promise after completion'],
    expectedDiagnostics: ['unsupported-store-feature'],
    forbiddenEdges: [{ kind: 'state-write', to: 'workspaceId',
      reason: 'loadScalars does not write the workspace selection' }] }),
  entry({ contract: 'R15', id: 'R15/rxjs.lastValueFrom', package: 'rxjs', export: 'lastValueFrom', version: RXJS,
    ...
    expectedEdges: ['http-consume|GET /api/metrics/sets|promise-consume',
  ...
  entry({ contract: 'R15', id: 'R15/rxjs.forkJoin', package: 'rxjs', export: 'forkJoin', version: RXJS,
    ...
    expectedEdges: ['http-create|src/api.ts#MetricsApi|GET /api/metrics/sets',
  ...
  entry({ contract: 'R15', id: 'R15/rxjs.of', package: 'rxjs', export: 'of', version: RXJS,
    expectation: 'ObservableInput の生成として既知アダプタに含める', fixture: rxjsConsume('data-id=ofButton'),
    expectedEdges: ['reactive-link|fromValues|of', 'reactive-link|of|distinctUntilChanged',
      'state-write|click → fromValues()|label'],
    expectedConditions: ['values are delivered when subscribed',
      'comparison may suppress an unchanged projected value'],
    expectedDetails: [{ edge: 'reactive-link|fromValues|of', key: 'operator', value: 'of' },
      { edge: 'reactive-link|of|distinctUntilChanged', key: 'operator', value: 'distinctUntilChanged' }] }),
  entry({ contract: 'R15', id: 'R15/rxjs.from', package: 'rxjs', export: 'from', version: RXJS,
    ...
    fixture: rxjsConsume('data-id=fromButton'),
    expectedEdges: ['reactive-link|fromPromise|from', 'state-write|click → fromPromise()|label'],
    expectedConditions: ['array, iterable, or Promise input is converted on subscription; Promise settles asynchronously'],
    expectedDetails: [{ edge: 'reactive-link|fromPromise|from', key: 'operator', value: 'from' }] }),
  entry({ contract: 'R15', id: 'R15/rxjs.distinctUntilChanged', package: 'rxjs', export: 'distinctUntilChanged',
    ...
    fixture: rxjsConsume('data-id=ofButton'),
    expectedEdges: ['reactive-link|of|distinctUntilChanged'],
    expectedConditions: ['comparison may suppress an unchanged projected value'],
    expectedDetails: [{ edge: 'reactive-link|of|distinctUntilChanged',
      key: 'scheduling', value: 'sync' }] }),
  entry({ contract: 'R15', id: 'R15/ngrx-operators.tapResponse', package: '@ngrx/operators', export: 'tapResponse',
    version: NGRX, expectation: '成功/失敗の分岐を保持する', fixture: rxjsConsume('data-id=tapButton'),
    expectedEdges: ['http-consume|GET /api/metrics/tapped|subscribe',
      'state-write|click → tapBoth()|total', 'state-write|click → tapBoth()|failed'] }),
  entry({ contract: 'R15', id: 'R15/ngrx-operators.mapResponse', package: '@ngrx/operators', export: 'mapResponse',
    ...
    fixture: rxjsConsume('data-id=mapButton'),
    expectedEdges: ['http-consume|GET /api/metrics/mapped|subscribe',
      'state-write|click → mapBoth()|total'],
    forbiddenEdges: [{ kind: 'state-write', to: 'failed',
      reason: 'mapResponse の error 分岐を tapResponse の分岐と混同しない' }] }),
