...
test('argument validation occurs before analysis', () => {
  ...
  assert.equal(parseArguments(['x=y', '--selector', 'body > app-root > input']).options.selector,
    'body > app-root > input');
...
test('DevTools selector chooses the matching component display path', () => {
  const copied = 'body > app-root > app-shell > div > app-search-area > as-split-area:nth-child(2) > ' +
    'app-search-page > app-search-form > app-inline-edit > div.input > form > input';
  assert.deepEqual(parseDomSelector(copied), ['body', 'app-root', 'app-shell', 'div',
    'app-search-area', 'as-split-area', 'app-search-page', 'app-search-form',
    'app-inline-edit', 'div', 'form', 'input']);
  assert.deepEqual(parseDomSelector('#app > app-inline-edit > input'), ['app-inline-edit', 'input']);
  const shared = ['app-root', 'app-shell'];
  const header = ['app-search-page', 'app-search-form', 'app-inline-edit'];
  const insideExperiments = { ...candidate(2), dom: { componentTags: [...shared, 'app-search-area', ...header],
    targetTag: 'input' } };
  ...
  assert.deepEqual(filterCandidates([insideExperiments, directOutput], {
    selector: 'app-inline-edit > div > form > input',
  }), [insideExperiments, directOutput]);
  assert.deepEqual(filterCandidates([insideExperiments], { selector: 'app-inline-edit > span' }), []);
